...
| Trackpad driver出力 | 25F80で`scroll`、`DockSwipe`、`NavigationSwipe`、`magnification`をsystem-wide投稿できる。`scroll`は確定contractと自前計測model、残る3 familyは候補raw fieldによる試用実装で、方向・係数の実機調整が残る | [ADR-0043](docs/adr/0043-trackpad-scroll-product-output.md)、[ADR-0046](docs/adr/0046-trial-output-for-remaining-trackpad-families.md) |
...
flowchart LR
    ...
    pass --> app
    session --> trial["25F80試用出力<br/>DockSwipe / NavigationSwipe / magnification"]
    trial --> post
...
| `run` | グローバルイベントタップで入力を読み、25F80の`scroll` / `DockSwipe` / `NavigationSwipe` / `magnification`製品output sessionへ接続する。未知OS buildやcontract不一致では入力抑制前に停止する |
...
25F80では4 familyがsupportedで、既定bindingのまま起動できます。`DockSwipe`、`NavigationSwipe`、`magnification`は候補raw fieldによる試用実装であり、Spaces / Mission Control、page navigation、zoomの方向と係数はNape Pro実機で調整します。`scroll`の確定contractと、残る3 familyの試用境界は[ADR-0043](docs/adr/0043-trackpad-scroll-product-output.md)と[ADR-0046](docs/adr/0046-trial-output-for-remaining-trackpad-families.md)を参照してください。
