...
Issue #6 / #12 の runtime event 証跡は `sh scripts/collect-runtime-event-evidence.sh` で収集します。総合状態は `status.json` の `success` / `blocked` / `failed` で確認し、人間作業は TCC 権限付与や実機操作など、自動化できない最後の手段に限定します。
