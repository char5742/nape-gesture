...
| Safari 操作 | 機械受入完了。単一候補`.app`で6 assertion / 12 runが全件passし、通常routing、PID固定sync / async、native wheel、frame端を確認。ページ戻る / 進む、ズーム、横スクロールの実画面とcurrent-uptime計画も再取得した | [Safari scroll 配送比較](docs/safari-scroll-delivery-verification.md)、[ADR-0035](docs/adr/0035-browser-discrete-shortcut-posting.md)、[ADR-0036](docs/adr/0036-scroll-event-target-process-routing.md)、[ADR-0037](docs/adr/0037-monotonic-event-time-domain.md)、[ADR-0038](docs/adr/0038-safari-evidence-pointer-and-routing.md) |
...
PR #101 の時刻修正前 Safari 診断は比較専用です。完成証跡は `artifacts/completion/2026-07-11/pr101-final-safari` と `pr101-final-runtime-event` に分け、ADR-0037 適用後の同一候補`.app`、current-uptime、画面差分、runtime completionを採用します。
...
AX scrollbar set は Web の `wheel` handler を発火せず、公開 AX tree が generic overflow の境界を省略する場合は通常 wheel と同じ nested routing を保証できません。成立範囲、Computer Use 比較、再現 fixture は [Safari scroll 配送比較](docs/safari-scroll-delivery-verification.md) を参照してください。runtime evaluatorは候補commitと`.app` executable identity、正本contractと全artifactのSHA-256、run専用path、実行argv、reset / before / after / atEnd、exit code、通常routing事前条件を検査します。`pr101-final-safari`では6 assertion / 12 runが全件`pass`で、`blocked`やPID固定診断を通常routing成功の代替にしていません。離散操作のComputer Use画面差分、current-uptime計画、横スクロール実画面、`pr101-final-runtime-event`のruntime performance completionも同じ候補で再取得します。
