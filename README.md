...
| Runtime event | `.build/NapeGesture.app` の TCC 許可済み経路で成功。2026-07-10 に main `e153f3c` 上で gesture-drag / gesture-wheel / kill-switch / gesture-wheel-then-kill-switch / normal-after-release を `scripts/collect-runtime-event-evidence.sh` で再取得し、`status: "success"` を確認した | [ADR-0032](docs/adr/0032-reference-target-foreground-capture.md)、[ADR-0033](docs/adr/0033-kill-switch-pending-release-suppression.md)、[docs/completion-checklist.md](docs/completion-checklist.md) |
