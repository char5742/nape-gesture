...
- `MacGestureCore`: ボタン押下中の移動・ホイールをジェスチャーへ変換する純粋ロジック
- `mac-gesture run`: グローバルイベントタップで入力を読み、生成スクロールイベントを投稿する常駐本体
- `mac-gesture log`: 実デバイス、純正トラックパッド、生成イベントを同じ JSON Lines 形式で記録するロガー
- `mac-gesture compare-log`: 純正入力ログと生成イベントログなど、2つの JSON Lines ログを比較する解析器
- `mac-gesture analyze-hid-log`: IOHID 生入力ログを device / usage ごとに集計する解析器
- `mac-gesture analyze-target-log`: Reference Target App が保存した AppKit 受信ログを集計する解析器
- `mac-gesture generate-scroll`: 任意のスクロールイベントを発火する生成器
- `mac-gesture target`: AppKit が受け取った `scrollWheel` / `swipe` / `magnify` などを表示し、JSON Lines に保存できる基準ターゲット
- `mac-gesture devices`: IOHID で認識できるマウス系または全 HID デバイスを一覧する補助コマンド
...
```sh
swift run mac-gesture app
swift run mac-gesture help
swift run mac-gesture devices
swift run mac-gesture devices --all --json
swift run mac-gesture check-config
swift run mac-gesture hid-log --duration 10
swift run mac-gesture hid-log --vendor-id <ID> --product-id <ID> --usage-page <ID> --usage <ID> --duration 10
swift run mac-gesture analyze-hid-log Fixtures/sample-hid-log.jsonl
swift run mac-gesture analyze-target-log Fixtures/sample-target-log.jsonl
swift run mac-gesture log
swift run mac-gesture log --duration 8 --out trackpad-space-right.jsonl --exclude-generated
swift run mac-gesture analyze-log Fixtures/sample-log.jsonl
swift run mac-gesture compare-log Fixtures/sample-trackpad-scroll-log.jsonl Fixtures/sample-generated-scroll-log.jsonl
swift run mac-gesture target
swift run mac-gesture target --out target-events.jsonl
swift run mac-gesture generate-scroll --x 0 --y -480 --steps 24
swift run mac-gesture generate-scroll --x 0 --y -480 --steps 24 --momentum-steps 12 --dry-run
swift run mac-gesture generate-scroll --x 0 --y -480 --steps 24 --momentum-steps 12 --dry-run --log-json > generated-scroll.jsonl
swift run mac-gesture generate-scroll --x 1200 --y 0 --steps 30 --mode space-right --phase auto --dry-run --json
swift run mac-gesture system-test list
swift run mac-gesture system-test run --scenario space-left --target finder --dry-run
swift run mac-gesture system-test run --scenario space-left --target finder --dry-run --log-json --out system-space-left.jsonl
swift run mac-gesture benchmark --events 200000
swift run mac-gesture doctor --probe-hid --benchmark-events 50000
swift run mac-gesture init-config --out mac-gesture.config.json
swift run mac-gesture init-config --vendor-id <ID> --product-id <ID> --usage-page <ID> --usage <ID> --out mac-gesture.config.json
swift run mac-gesture run
swift run mac-gesture bundle-app --out .build/MacGesture.app --replace
swift run mac-gesture verify-bundle .build/MacGesture.app
swift run mac-gesture-core-tests
```

`run` と `log` はグローバル入力を扱うため、アクセシビリティ権限が必要です。
`log` は `--duration <秒>` で自動停止、`--out <path>` で JSON Lines を保存します。開始・終了などのメタ情報は標準エラーに出し、イベント本体だけを標準出力または `--out` に出します。`--exclude-generated` は純正入力や実デバイス入力だけ、`--only-generated` は Mac Gesture が生成したイベントだけを記録します。
...
`run`、`check-config`、`app` は `--config` を省略した場合、`~/Library/Application Support/MacGesture/config.json` を使います。存在しない場合は Nape Pro 向けテンプレートを作成します。対象デバイス一致が必須のまま対象条件が空の場合は、全デバイスへ誤適用しないよう起動前に停止します。
...
```sh
swift run mac-gesture check-config --probe-hid
```

`kIOReturnNotPermitted` が出る場合は、システム設定の「プライバシーとセキュリティ」で、実行元の Codex、ターミナル、または `MacGesture.app` に「入力監視」を許可してください。
権限を付与した直後に macOS が反映しない場合は、実行元アプリまたは `MacGesture.app` を再起動してください。常駐UIは再起動後の初回起動で再度開始し、失敗時は自動再試行状態へ入ります。
...
作成した `MacGesture.app` は引数なしで起動するとメニューバー常駐UIとして動きます。アクセシビリティや入力監視の許可は、この `.app` に対して付与してください。

## 検証方針

詳細な実機検証手順、完成判定に必要な証跡、既知の失敗条件と回復手順は `docs/verification.md` にまとめています。

1. `mac-gesture log --duration <秒> --out <path>` で純正トラックパッド、Nape Pro、生成イベントを同じ形式で記録する
2. `mac-gesture analyze-log <path>` で移動量分布と `deadZonePoints` 候補を確認する
3. `mac-gesture compare-log <純正ログ> <生成ログ>` でイベント数、precise 率、フェーズ分布、スクロール総量の差を確認する
4. `mac-gesture hid-log` と `mac-gesture analyze-hid-log <path>` で Nape Pro の HID usage と値域を確認する
5. `mac-gesture target --out <path>` で AppKit に届くイベント差分を画面と JSON Lines の両方で確認し、`analyze-target-log <path>` で集計する
...
- `docs/naming-migration.md`: `Mac Gesture` / `mac-gesture` から `nape-gesture` へ移行する際の命名リスク
