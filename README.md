...
`--ready-file <path>` を指定すると、1件目の有効 CPU sample を採取し、その前後で identity が一致した直後に、`schemaVersion`、`ready`、`completedSampleCount: 1`、PID、開始トークン、pidversion、resolved path、timestamp を含む完全な JSON を公開します。JSON は一時 sibling へ書き終えてから macOS の `renamex_np(..., RENAME_EXCL)` で final path へ排他的かつ atomic に公開し、競合時は既存ファイルを上書きせず明示的に失敗して一時ファイルを削除します。`--out` と ready path は component を左から処理し、存在する symlink をその地点で target へ展開してから後続の `..` を適用します。非存在 component、相対 path、root、大文字小文字差も安全側で正規化するため、`alias/link -> ../real/child` のときの `alias/link/../report.json` と `real/report.json`、`--out Report.JSON` と `--ready-file report.json` のような衝突も拒否します。ready は1件の有効 sample 完了だけを示し、duration 到達や baseline 合格は示しません。
