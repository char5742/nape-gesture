...
| System Behavior Test | readiness matrix を CLI から JSON / Markdown で出力し、全 scenario の前段機械証跡、runtime / target 証跡、Issue 対応、画面挙動実測待ちを分離する。`need:human` は物理操作など不可避な外部作業だけに限定する | `system-test readiness --json --assert`、[docs/system-behavior-matrix.md](docs/system-behavior-matrix.md) |
...
swift run nape-gesture system-test run --scenario horizontal-scroll --post-to-pid <Reference Target App PID>
