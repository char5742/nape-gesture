...
ここでの「2 / 3 / 4本指」はraw digitizer contact数でもgeneric `fingerCount` fieldでもありません。物理trackpad driverがgestureを認識した後に上位へ生成する固定GestureClassを表します。3本指DockSwipeと4本指DockSwipe motion 4はevent family、motion、axis、符号規則を含むencodingが異なりますが、mouse source deltaとsource velocityには同じ`/ 600`の固定変換スケールを使います。このスケールはユーザー変更可能な感度ではなく、button 5はapplication magnificationを使いません。
