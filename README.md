...
buttonとgesture classの対応は製品仕様として固定です。mode、割り当て、方向別設定、application別設定はありません。button 4の3本指system swipeとbutton 5の4本指system pinchにだけ共通する「システムジェスチャー感度」は、GUIで25%から200%まで設定でき、既定値は100%です。button 3の2本指scrollには適用しません。
...
ここでの「2 / 3 / 4本指」はraw digitizer contact数でもgeneric `fingerCount` fieldでもありません。物理trackpad driverがgestureを認識した後に上位へ生成する固定GestureClassを表します。3本指DockSwipeと4本指DockSwipe motion 4はevent family、motion、axis、符号規則を含むencodingが異なります。100%では従来どおりsource deltaとsource velocityを`/ 600`で変換し、実際の変換値は`(source / 600) * システムジェスチャー感度倍率`です。button 5はapplication magnificationを使いません。
...
設定ウィンドウは、日常的に確認する「ジェスチャー」と低頻度の「詳細」をtoolbarで分離します。「ジェスチャー」paneではruntime状態、通常mouseへの復帰条件、次の固定対応を読み取り専用で表示し、共通の「システムジェスチャー感度」を編集できます。

- button 3 = 2本指スクロール / スワイプ
- button 4 = 3本指システムスワイプ
- button 5 = 4本指system pinch

「システムジェスチャー感度」はcanonical設定`gesture.systemGestureSensitivity`へ倍率0.25から2.0として保存し、1.0を100%として扱います。button 4 / 5に同じ倍率を適用し、button別、方向別、application別の感度には分けません。buttonごとのmode selector、無効化、割り当て、方向別binding、application別設定もありません。`dragSensitivity`、`wheelSensitivity`などの旧調整値は新しい倍率へ移行せずcanonical設定から除去し、新しいcanonical値がない旧設定には1.0を補います。移行失敗時は原本を保持してruntimeを開始しません。「詳細」paneにはgestureの意味を変更しないcancel条件と対象device条件だけを置き、低レベルの識別条件は開示するまで隠します。変更がある場合だけ「変更を適用」を有効にし、適用時にruntimeを再起動します。「権限とデバイス」にはAccessibility、Input Monitoring、対象device、macOS version / build、output contract / fixture、必須family、runtime状態、fail-closed理由を表示します。メニューバーには文字列`NG`ではなく、accessibility label付きのsystem symbolを表示します。
...
| 設定 | 共通感度の範囲・既定値・旧感度非移行、旧設定の原子的migration、冪等性、不正原本の保持、process間排他、lock file改変拒否、保存失敗後の再試行 |
| GUIとdoctor | 固定mappingの読み取り専用表示、共通感度slider、dirty / revert、pane / disclosure、複数device条件保持、複合HIDのマウスインターフェース限定、起動identityとreadinessの整合 |
...
| GUI / migration / doctor | 固定mappingと共通のシステムジェスチャー感度へ更新。保存失敗時の再試行、process間排他、曖昧な起動identityとinventory失敗の明示まで検証 |
...
- システムジェスチャー感度が25%から200%、既定100%でbutton 4 / 5だけへ共通適用され、button 3と固定mappingを変更しない。
