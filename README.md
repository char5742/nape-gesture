...
| Trackpad driver出力 | 25F80で4 familyをsystem-wide投稿できる。製品runtimeの固定入力はmouse move→`DockSwipe`、wheel→2次元`scroll`。`NavigationSwipe`と`magnification`は生成能力のみで、方向別bindingではない起動操作が未確定 | [ADR-0043](docs/adr/0043-trackpad-scroll-product-output.md)、[ADR-0046](docs/adr/0046-trial-output-for-remaining-trackpad-families.md)、[ADR-0047](docs/adr/0047-fixed-continuous-2d-trackpad-input.md) |
...
- 設定ウィンドウで activation button、感度、加速度、慣性、キャンセル条件、対象デバイス、対象入力の紐づけ秒を編集する
...
方向別またはアプリごとのジェスチャー割り当ては持ちません。
特定ボタンを押していない状態では通常マウスとして振る舞うため、アプリ別に通常入力へ戻す設定は不要です。
特定ボタンの押下中は、mouse moveを開始時の優勢軸に固定した3本指`DockSwipe`、wheelを2次元`scroll`として変換します。途中で方向が反転しても別actionや別sessionへ切り替えません。`NavigationSwipe`と`magnification`は方向別bindingへ接続しません。
...
| `run` | グローバルイベントタップで入力を読み、mouse moveを`DockSwipe`、wheelを`scroll`製品sessionへ接続する。未知OS buildやcontract不一致では入力抑制前に停止する |
...
25F80では4 familyを生成できますが、製品runtimeの固定入力はmouse move→`DockSwipe`、wheel→`scroll`です。`NavigationSwipe`と`magnification`は候補raw fieldによる生成能力を維持する一方、方向別bindingではない起動操作が未確定なので通常runtimeから到達可能とは扱いません。固定操作体系は[ADR-0047](docs/adr/0047-fixed-continuous-2d-trackpad-input.md)、`scroll`の確定contractと残る3 familyの試用境界は[ADR-0043](docs/adr/0043-trackpad-scroll-product-output.md)と[ADR-0046](docs/adr/0046-trial-output-for-remaining-trackpad-families.md)を参照してください。
