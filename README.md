...
| Trackpad driver出力 | 25F80で4 familyをsystem-wide投稿できる。button 3 / 4 / 5ごとに`none`、`Scroll & Navigate`、`Spaces & Mission Control`、`Zoom`を割り当てる。`NavigationSwipe`はadapter生成能力であり、ページnavigationの完成保証ではない | [ADR-0043](docs/adr/0043-trackpad-scroll-product-output.md)、[ADR-0046](docs/adr/0046-trial-output-for-remaining-trackpad-families.md)、[ADR-0047](docs/adr/0047-fixed-continuous-2d-trackpad-input.md) |
...
25F80では4 familyを製品sessionへ接続します。`scroll`は確定済み、残る3 familyは試用段階です。未知OS buildやcontract不一致ではevent tapと入力抑制を開始しません。
...
flowchart LR
    ...
    contract["adapter単体生成能力<br/>NavigationSwipe / magnification"] -.-> post
...
Spaces / Mission Control、page navigation、zoomの方向と体感を含む完成判定は、[docs/completion-checklist.md](docs/completion-checklist.md)を正本にします。
...
- 設定ウィンドウでbutton 3 / 4 / 5ごとのmode、感度、加速度、慣性、キャンセル条件、対象デバイス、対象入力の紐づけ秒を編集する
...
上下左右の方向別またはアプリごとのジェスチャー割り当ては持ちません。代わりにbutton 3 / 4 / 5ごとに、通常mouseとして扱う`none`、`Scroll & Navigate`、`Spaces & Mission Control`、`Zoom`のmodeを選びます。既定はbutton 3=`Scroll & Navigate`、button 4=`Spaces & Mission Control`、button 5=`Zoom`です。
特定ボタンを押していない状態では通常マウスとして振る舞うため、アプリ別に通常入力へ戻す設定は不要です。
`Scroll & Navigate`は押下中のmouse moveとwheelを2次元`scroll`として扱います。`NavigationSwipe` adapterには生成能力がありますが、この能力やmode名だけでページ戻る/進むが完成したとは扱いません。`Spaces & Mission Control`はmouse moveを開始時の優勢軸に固定した3本指`DockSwipe`、`Zoom`はmagnification系列へ接続します。途中で方向が反転しても別modeや別sessionへ切り替えません。
...
25F80では4 familyを生成できます。製品runtimeはbuttonごとのmodeへ接続しますが、`scroll`だけが確定contractで、`DockSwipe`、`NavigationSwipe`、`magnification`は候補raw fieldによる試用能力です。特に`Scroll & Navigate`の`scroll`経路と`NavigationSwipe` adapter能力は同一ではなく、後者の存在だけでページnavigationを完成扱いにしません。操作体系は[ADR-0047](docs/adr/0047-fixed-continuous-2d-trackpad-input.md)、`scroll`の確定contractと残る3 familyの試用境界は[ADR-0043](docs/adr/0043-trackpad-scroll-product-output.md)と[ADR-0046](docs/adr/0046-trial-output-for-remaining-trackpad-families.md)を参照してください。
