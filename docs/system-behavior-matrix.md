...
- `system-test readiness --json --assert` の成功は、シナリオ定義、exact Issue 紐づけ、expected target、event shape、機械証跡コマンド、runtime / target 証跡、画面挙動待ち、`need:human` 境界が欠落していないことを示す。
...
| `completionState` | 画面挙動待ちは `screen-behavior-evidence-pending`、runtime / target 証跡だけが残る場合は `runtime-target-evidence-pending`、両方ない場合は `machine-evidence-complete` |
...
| `scenarios[].completionRequirement` | scenario が満たす完成要件 |
| `scenarios[].relatedIssues` | scenario ごとの exact Issue 集合。Nape Pro 実機境界を持つ scenario は #4 も含む |
| `scenarios[].expectedTargets` | foreground App または Reference Target App などの期待対象 |
| `scenarios[].eventShape` | 生成または未生成イベント列の意味 |
| `scenarios[].machineEvidence` | completion evidence で先に保存するコマンド |
| `scenarios[].runtimeEvidence` | daemon、target log、PID target など runtime / target で必要な証跡 |
| `scenarios[].screenBehaviorEvidence` | 実画面で最終確認する内容 |
| `scenarios[].humanWorkBoundary` | 人間作業へ進む前に代替する確認 |
| `scenarios[].completionState` | scenario 単位の画面挙動待ち、runtime target 証跡対象、機械前段証跡対象 |
...
`space-left` / `space-right` は Reference Target App PID へ直接投稿し、ready diagnostics と生成 foreground capture の方向 / 件数 / 量も保存する。
...
`horizontal-scroll` は Reference Target App PID へ直接投稿し、ready diagnostics と生成 foreground capture の方向 / 件数 / 量も保存する。離散 shortcut 系は PID target 証跡にしない。
