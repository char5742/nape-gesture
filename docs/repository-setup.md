...
製品挙動は[ADR-0049](adr/0049-fixed-button-to-gesture-class-input.md)に従い、次のbutton割り当てモデルを正とする。
...
- 選択したGestureClassから既存ProductOutputへのevent contractは変更しない
- button 3 / 4 / 5未押下時は通常mouse入力を変更せず通す
- button 3 / 4 / 5以外のbuttonと対象外deviceも通常mouse入力として変更せず通す

「2 / 3 / 4本指」はraw contact数やgeneric `fingerCount` transportではなく、GestureClassの説明である。classごとにevent type、field、phase、companion、単位変換が異なる。結果別mode、方向別action、application別設定、button別感度は製品モデルに含めず、最終結果はmacOSまたは前面applicationが解釈する。共通の`systemGestureSensitivity`は物理button番号ではなく選択された3本指 / 4本指classへ適用する。

baseline `55eb991` のbutton別結果mode選択と、その後の変更不能な固定mappingはどちらも移行前履歴であり、現在の製品仕様を示さない。旧mode test、固定mappingの実機証跡、個別familyの投稿成功、`doctor`、`.app`生成または署名済みappの試用だけを完成根拠にせず、現行の要件と完成判定は[ゴール要件](requirements.md)、[ADR-0049](adr/0049-fixed-button-to-gesture-class-input.md)、[完成判定チェックリスト](completion-checklist.md)を正本にする。
...
Issue 7のcloseは移行前の診断上の狭い成果を表すだけであり、3つのGestureClass、button割り当て、未押下pass-through、class固有ProductOutput、または製品全体の完了を表さない。後続のclose済みIssueも、旧mode / 固定mapping / familyスコープの証跡だけなら同様に現行完成判定へ流用しない。
...
メインスレッドは、OS/Appの個別結果を製品modeへ昇格させず、button→保存済みGestureClass割り当て→class固有ProductOutputが設定、UI、migration、runtime、テスト、文書、物理受入で一致するまで完成判定を保留する。
