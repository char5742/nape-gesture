...
本証跡は、Mac Mouse Fixや生成eventの値を正本にせず、`physicalTrackpad` captureだけからfieldと順序を再導出する。
...
manifestの開始・終了wall-clock、source log SHA-256、prefix件数、target件数は[小型fixture](../../Fixtures/trackpad-contract/25F80/physical-observations.json)へ保存した。
...
`ruby scripts/verify-trackpad-physical-observations.rb --json`は8本とlegacy 1本を再読込し、全SHA、event数、capture wall-clock、keyboard境界prefix、target件数、生成marker 0件、scroll companion 1,165組とcaptureIndex差`-1 / +2 / +3 / +4`、timestamp非同値をfixtureから再導出して成功した。
