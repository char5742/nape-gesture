...
PID を特定し、1 秒間隔で CPU を採取する。

```sh
pid=$(pgrep -x nape-gesture | head -n 1)
.build/debug/nape-gesture sample-cpu --pid "$pid" --duration 30 --interval 1 --mode idle --json --assert-baseline > cpu-idle.json
.build/debug/nape-gesture sample-cpu --pid "$pid" --duration 30 --interval 1 --mode active --json --assert-baseline > cpu-active.json
.build/debug/nape-gesture sample-cpu --pid "$pid" --duration 5 --interval 1 --mode recovery --json --assert-baseline > cpu-recovery.json
```
...
`sample-cpu` の `measurementKind` は `processCpuSampling`、`includesEventTapAndPosting` は `false` である。
