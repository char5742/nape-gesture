...
- `sample-cpu --expected-executable <path> --json --assert-baseline` で単調 uptime の duration coverage、開始トークン、実行ファイルパス、audit token の `(pid, pidversion)` による実行主体同一性、初回有効 sample 後の排他的 ready handshake、idle / active / recovery の CPU 使用率を終了コード判定できる
