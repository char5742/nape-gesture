...
時刻基準:
Issue #102 / ADR-0037 で `CGEventTimestamp`、慣性、生成ログを起動後単調時刻へ統一済みである。統合前の生成ログは epoch 混入有無を確認し、純正比較の完成証跡には統合 commit で再取得したログを使う。
...
時刻基準:
PR #101 の統合前 Safari 診断は epoch timestamp 投稿が混在した可能性を除外できない。Issue #102 / ADR-0037 は統合済みだが、その統合 commit で CGEvent log と画面挙動を再取得するまで、ページ戻る / 進む、ズーム、縦横スクロールを完了扱いにしない。
...
時刻基準:
PR #101 を含む時刻修正前の Safari 証跡は Issue #10 の完了根拠から外す。Issue #102 / ADR-0037 の統合 commit で再取得した CGEvent log に `--assert-current-uptime` を通し、computer-use の画面挙動と同じ実行 commit を対応付けてから再採用する。
