...
- 水平と垂直のsource入力をIOHID `DockSwipe` motion 1 / 2、累積progress、XY position、終端XY velocityへ変換する。motionとprogress増分はsource delta / 600、終端velocityはsource velocity / 600を基準とする。
...
- phase fieldsは3本指classと同じ1 / 2 / 4 / 8を使う一方、XY positionと終端XY velocityは0、pinch progressはYが非0なら`-Y / 600`、それ以外は`X / 600`を累積し、同じ符号規則のsource velocity / 600を終端Z velocityへ設定する。
