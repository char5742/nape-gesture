...
Issue #10 のページ戻る、進む、ズーム、縦横スクロールは Safari / 対応アプリでの画面挙動確認が必要である。2026-07-10 時点で、権限付与済みの `.build/NapeGesture.app/Contents/MacOS/nape-gesture` から Safari へ投稿し、`page-back` / `page-forward` / `zoom-in` / `zoom-out` と top-level 縦横スクロールを確認済みである。ただし top-level の変化だけを Web scroll 完成証跡にしない。[Safari scroll 配送比較](safari-scroll-delivery-verification.md) と `docs/fixtures/safari-scroll-probe/contract.json` の5 assertionで、unlabeled generic overflow の fail-closed、縦のみ frame への微小横成分混在、通常の長い article、`AXWebArea` 自身の scrollbar、Computer Use の wheel handler を分けて比較する。今回のローカル Safari では generic 全値 0、frame `y 0 -> 367` / outer 0、article outer `0 -> 674`、top-level `x 0 -> 1358 / y 0 -> 608` を確認した。contract の静的整合は `python3 scripts/check-safari-scroll-probe-contract.py` で completion に固定するが、Issue #102 取り込み後の最終 Safari runtime 証跡の代替にはしない。AX scrollbar set は wheel handler を発火せず、公開 AX tree が generic container を完全に省略する場合の通常 wheel 同等 semantics は未成立である。
...
Issue #102 の時刻ドメイン修正はこの branch へ重複実装しない。PR #101 の最終 Safari / runtime 証跡は Issue #102 取り込み後の head で再取得する。
