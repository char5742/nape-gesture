...
Issue #10 のページ戻る、進む、ズーム、横スクロールは Safari / 対応アプリでの画面挙動確認が残るため、`page-back`、`page-forward`、`zoom-in`、`zoom-out`、`horizontal-scroll` の dry-run と `analyze-log --assert-current-uptime --assert-system-scenario` は実機前証跡であり、完了扱いにはしない。PR #101 の Safari 診断は修正前の epoch timestamp 投稿が混在した可能性を除外できないため、Issue #10 / #16 の証跡は ADR-0037 適用後の CGEvent log、画面挙動、実行 commit をそろえて再取得する。
...
`system-test run --post-to-pid <pid>` は Reference Target App の sink 診断専用である。completion evidence では `.cghidEventTap` 経由の通常 `system-test run` を使う。
