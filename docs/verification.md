...
現在のproduct capabilityは`scroll` familyだけをsupportedにする。`DockSwipe`、`NavigationSwipe`、`magnification`は未実装であり、既定bindingを含め未対応familyが必要な設定では`outputContract.missingFamilies`としてevent tap開始前に停止する。したがって、scroll adapterの実装をアプリ全体の完成、#125、OS build展開、Nape Pro実機比較、公証の完了とは扱わない。
