...
アクセシビリティと入力監視はどちらも `.app` 経路で許可済み、HID 入力監視プローブは成功、`runtimeReadiness.ready` は `true` である。
...
ただし、この証跡は Issue #102 の時刻修正前であり、実入力終了後の最初の慣性 tick が `.momentum` になることまでは証明しない。元入力抑制など時刻不整合に依存しない判定と、修正後に再取得する慣性・Safari 証跡を分けて扱う。
...
.build/debug/nape-gesture analyze-log generated-space-right.jsonl --json --assert-current-uptime
.build/debug/nape-gesture analyze-log system-space-right.jsonl --json --assert-current-uptime --assert-system-scenario space-right
.build/debug/nape-gesture analyze-log system-horizontal-scroll.jsonl --json --assert-current-uptime --assert-system-scenario horizontal-scroll
...
有限入力から派生値がoverflowする境界も、部分的なJSON Linesを残さず失敗することを確認する。

```sh
if .build/debug/nape-gesture generate-scroll --x 1e308 --steps 1 --momentum-steps 1 --momentum-scale 2 --dry-run --log-json > generated-overflow.jsonl; then
  exit 1
fi
test ! -s generated-overflow.jsonl

if .build/debug/nape-gesture generate-scroll --x 1 --steps 2 --interval 1e308 --dry-run --log-json > generated-unconvertible-timestamp.jsonl; then
  exit 1
fi
test ! -s generated-unconvertible-timestamp.jsonl
```

### 起動後単調時刻の検証

イベント時刻の正本は [ADR-0037](adr/0037-monotonic-event-time-domain.md) とする。

| 値 | 時刻ドメインと単位 |
| --- | --- |
| `CGEvent.timestamp` / `InputLogRecord.timestamp` | 起動後ナノ秒 |
| `RawInputEvent.time` / `GestureCommand.timestamp` / `RuntimePerformanceRecord.commandTimestamp` | 起動後秒 |
| runtime performance の `*Nanoseconds` | 起動後ナノ秒 |
| Reference Target ready file の `wallClockUnixSeconds` | Unix epoch 秒。イベント時刻との照合に使わない |

`--assert-current-uptime` は、全レコードの `timestamp` が analyzer 実行時の uptime から60秒以内であることを確認する。
epoch 混入を検出する取得直後専用 assertion であり、保存後60秒を超えたログの一般解析には付けない。
`Fixtures/epoch-timestamp-generated-scroll-log.jsonl` は expected failure とし、Unix epoch ナノ秒を成功扱いしない。

アクセシビリティ許可済み環境で実投稿 timestamp まで確認する場合は、投稿と同時に CGEvent log を取得する。

```sh
generated_log=/tmp/nape-generated-uptime.jsonl
rm -f "$generated_log"
.build/debug/nape-gesture log --duration 4 --out "$generated_log" --only-generated &
logger_pid=$!
sleep 0.2
.build/debug/nape-gesture generate-scroll --x 0 --y -240 --steps 8
wait "$logger_pid"
.build/debug/nape-gesture analyze-log "$generated_log" --json --assert-current-uptime
```

`system-test` も同様に実投稿と `log` を重ね、取得直後に `--assert-current-uptime` を実行する。
dry-run は同じ計画時刻と変換 API を使うため completion collector で常時検査するが、Safari 画面反映、AppKit 受信、実投稿の代替証跡にはしない。
...
`system-test run --dry-run --log-json` は、実イベントを投稿せず、System Behavior Test が生成する予定のスクロールまたはショートカットイベントを同じ JSON Lines 形式で保存する。最大256 step、系列全体30秒以内とし、派生 delta、速度、全 timestamp を1件目の投稿前に検証する。256 step は AX 同期探索の1 event 120ms上限を含めても CGEvent の60秒 skew境界に余裕を残す上限である。JSON Lines は全レコードの encode 成功後だけ stdout または `--out` を更新し、失敗時は既存ファイルを置換しない。dry-run レコードには `systemTestScenario` と `sequenceIndex` が入り、`analyze-log --json --assert-system-scenario <name>` でシナリオ名と順序も確認する。Spaces のスクロール系シナリオは `compare-log` の候補ログとして使い、`horizontal-scroll` は通常の横スクロール割り当ての scrollWheel イベント列として `analyze-log --json --assert-system-scenario horizontal-scroll` で水平 delta、phase、precise/continuous、`momentumPhase == 0` を確認する。実投稿の連続scrollは端到達や一時`noChange`で系列を打ち切らず全stepを実行し、CGEvent作成失敗がなく系列全体で1件以上配送できた場合だけ成功、全step 0件は配送不能として失敗にする。Mission Control やページ戻る/進むなどの離散シナリオは `analyze-log --json --assert-system-scenario <name>` で修飾キー down/up を含む完全な key event 列、keyCode、exact modifier flags、generated marker を確認する。
`kill-switch` の単独シナリオも、実投稿時は他の未マーク入力シナリオと同じイベント列を使い、`keyDown` / `keyUp` の間隔を `interval` で明示する。ゼロ間隔の合成ショートカットは daemon の停止ログ確認に使わない。
Issue #10 のページ戻る、進む、ズーム、縦横スクロールは Safari / 対応アプリでの画面挙動確認が必要である。`page-back`、`page-forward`、`zoom-in`、`zoom-out`、`horizontal-scroll` の dry-run と `analyze-log --assert-current-uptime --assert-system-scenario` は実操作前証跡であり、単独では完了扱いにしない。[Safari scroll 配送比較](safari-scroll-delivery-verification.md) と `docs/fixtures/safari-scroll-probe/contract.json` の6 assertion / 12 runで、unlabeled generic overflow の fail-closed、native wheel、縦のみ frame への微小横成分混在、通常の長い article、`AXWebArea` 自身の scrollbar、frame端到達を分けて比較する。時刻修正前に得た generic / frame / article / top-level の値は selector 比較専用であり、ADR-0037 統合 head の完成証跡には流用しない。静的整合は `python3 scripts/check-safari-scroll-probe-contract.py`、実fixtureの初期値とframe終端は `swift scripts/check-safari-scroll-probe-render.swift`、合成artifactの正負・blocked回帰は `python3 scripts/check-safari-scroll-runtime-evidence-tests.py`、実artifactはevaluatorへ`--expected-commit`と`--app-executable`を指定して判定する。実artifactはexit `0`、`status=pass`、`failureCount=0`、`blockedCount=0`を必要条件とし、exit `2`のhost overlayやPID固定診断を通常routing成功へ昇格させない。正確なコマンドは[Safari scroll 配送比較](safari-scroll-delivery-verification.md)を正とする。これらはCGEvent logとSafari画面挙動の代替にはしない。AX scrollbar set はwheel handlerを発火せず、公開AX treeがgeneric containerを完全に省略する場合の通常wheel同等semanticsは未成立である。
runtime では AX fallback、同じ入力の CGEvent fallback、離散ショートカットを serial queue に載せ、AX request がない Spaces は同期直送する。daemon 起動時と activation button 押下時に水平 / 垂直 Web capability を全体 40ms で prewarm し、1 call 20ms、実配送探索 120ms、async 最大待ち 160ms、window target cache 500ms、normalized value cache 250ms とする。AX target は application hit-test から毎 step 再解決し、container identity を value cache key に含める。ancestor完全走査済み `notHandled` だけは PID / window / exact pointer / requested axes で500ms保持し、`found` / `blocked` で破棄する。generic container は `AXDescription` に依存せず全 direct children の clipping を調べ、frame の欠落・非有限値・負寸法を含む情報不足は `blocked` にする。最も近い target の利用可能軸だけを配送し、未対応軸は同じイベントの outer / CGEvent へ流さない。root hit-test の cold `kAXErrorCannotComplete` は deadline 内で1回だけ再試行する。positive `blocked`、`noChange`、適用済みは fallback を抑止し、root hit-test を2回とも扱えない場合または完全走査済み non-Web の `notHandled` だけが CGEvent fallback を許可する。runtime 性能ログは schema 2 の provisional と queue 内 completion を command ID で解決し、completion がない deferred record は baseline を通さない。queue 内 completion はAX適用・rollback不能の部分適用を実配送1件、`blocked` / `noChange` を0件として実結果を記録し、enqueue時の provisional 件数を最終成功として流用しない。`pr101-ax-delivery-final6` の p95 0.10〜1.76ms は AppKit 受信、nested target、Safari 画面反映、wheel handler を含まない。
Codex などの自動化ホスト window が Quartz 上の実ポインタ位置を覆う場合は、`generate-scroll --post-to-pid <Safari PID>` で診断対象だけを固定できる。この override は画面証跡取得用であり、通常 runtime のポインタ直下 window owner 選択を変更しない。value option の値欠落、未知・重複 option、余分な positional argument は `sh scripts/check-generate-scroll-cli.sh .build/debug/nape-gesture` で expected failure にする。
ADR-0037 はこの branch へ統合済みである。最終 Safari / runtime 証跡は `artifacts/completion/2026-07-11/pr101-final-safari` と `pr101-final-runtime-event` に保存し、同じ候補commitとTCC許可済み`.app` identityを固定する。contractの6 assertion / 12 run、通常async、PID固定sync / async、frame端到達、Computer Useの通常wheel、離散操作のcurrent-uptime計画と画面挙動、runtime performance completionを再取得する。manifest schema 2で候補commit、app executable、正本contract、全artifactのSHA-256、run専用path、実行argvを固定する。
...
実アプリ挙動の採否では TCC 主体を合わせるため、debug CLI や一時 Swift バイナリではなく、権限付与済みの `.app` bundle から実行する。Safari のページ戻る / 進むは `Command + [` / `Command + ]`、ズームインは `Command + Shift + =`、ズームアウトは `Command + -` を期待値にする。
...
`target --focus-capture-point` は検証自動化用に capture view 中心へカーソルを移動し、ready file の `focus` に AppKit screen 座標、Quartz 座標、移動後 cursor location を残す。手動比較では不要だが、`system-test` は現在のポインタ位置へ未マークイベントを投稿するため、無人 target log 証跡ではこのオプションを使い、要求 Quartz 座標と実 cursor location が2pt以内であることも確認する。`gesture-drag` の無人証跡は capture window 外へカーソルを押し出さないよう `--amount 240` を固定する。
...
`system-test run --post-to-pid <pid>` は未マーク通常入力の Reference Target App sink に固定する CLI 診断 override である。通常 runtime の生成イベントはこのフラグを使わず、[ADR-0036](adr/0036-scroll-event-target-process-routing.md) に従ってポインタ直下 window owner PID へ内部配送する。
...
8. `mission-control`、`space-right`、Safari のページ戻る/進む、ズームも同じ基準で確認し、各 CGEvent log に `analyze-log --assert-current-uptime` を通す。PR #101 の修正前診断は再利用しない
