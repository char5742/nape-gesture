...
- HID Button の press / release、Generic Desktop X / Y / Wheel、ゼロ値無視、runtime 非対応 AC Pan 無視を `HIDTargetActivityMapper` で固定する
- 設定値が `TargetDeviceGate` に反映される
- 設定値を超えたクリック、ドラッグ、ホイールは対象外入力として処理しない
- HID レポートの OS AbsoluteTime を Mach timebase で秒へ変換し、callback 実行時刻ではなく event tap と同じ単調時刻基準で比較する
- activation button down / up、pointer、wheel を別候補として扱い、異種 HID 活動では event tap 入力を許可しない
- event tap と HID は `0 <= eventTapTime - hidTime <= associationWindow` だけを関連付け、未来の HID 候補を `abs` 差で採用しない
- non-target の同種候補を観測したら target 候補を破棄し、その timestamp から `associationWindow` 満了まで種別単位で隔離する。隔離中、候補 overflow、候補再利用は通常入力として通す
- target 候補は bounded queue で保持し、同じ HID report timestamp の X / Y を一つの cohort として 1 回だけ消費する
- 進行中ジェスチャーで同種競合を検出したら recognizer と慣性を cancel し、同じ物理押下中に再開しない
- 対象デバイスのジェスチャーボタン release は、受理済みの対象 button down だけが解放待ちを作る。対象 HID release 候補がある event tap release だけを処理し、HID 証拠より先着した release は通常通過と内部 cancel で stuck を防ぐ
- non-target の非 activation button はイベント自体を処理せず、進行中ジェスチャーと慣性だけを cancel する
- 対象デバイス切断時は gate、進行中ジェスチャー、慣性を reset / cancel する
- `cancel` は状態復旧のため常に処理し、activation button 以外の button は押下中でもジェスチャー処理へ巻き込まない
- 古い設定JSONは `0.12` 秒を補い、既存挙動を維持する
- `0` 以下または非有限値は設定検証で拒否する

実機で完了判定するには、Nape Pro 操作時、通常入力デバイス操作時、両者を近接して操作した場合の HID ログとイベントタップログを同じシナリオで保存する。`analyze-association` で時刻差分布、associationWindow 内外件数、推奨 `associationWindow` を確認し、non-target 同種 HID 後の quarantine 中は target を含めて通常入力として通り、満了後の target 入力だけが再び処理されることを Reference Target App または System Behavior Test で確認する。
特に、target HID 候補の直後に通常マウスの同種操作を行い、non-target CGEvent が HID callback より先着する race で通常入力が抑制されないかを target log と HID log の両方で確認する。この race が未測定の間は Issue #5 と対応 PR を完了扱いにしない。
