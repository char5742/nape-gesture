...
ADR-0037 はこの branch へ統合済みである。最終 Safari / runtime 証跡は同じ統合 commit と TCC 許可済み `.app` identityを固定し、contractの6 assertion / 12 run、通常async、PID固定sync / async、frame端到達、Computer Useの通常wheel、生成CGEvent logの`--assert-current-uptime`、画面挙動、runtime performance completionを再取得する。manifest schema 2で候補commit、app executable、正本contract、全artifactのSHA-256、run専用path、実行argvを固定する。
