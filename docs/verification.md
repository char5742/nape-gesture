...
保存した AppKit 受信ログは `analyze-target-log <path>` で集計し、`scrollWheel`、`swipe`、`magnify`、`rotate`、phase、momentumPhase、precise scroll の有無を確認する。生成 foreground スクロールは `canonicalGeneratedForegroundCapture*` の値で `sendEvent` と `localMonitor` の二重記録を排除して確認する。
