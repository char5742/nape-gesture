...
常駐 CPU 使用率は、対象 runtime PID に `sample-cpu --json --assert-baseline` を実行して保存する。completion evidence の短時間 smoke や `/bin/sleep` PID への実行は、コマンド形式の退行確認であり、常駐 CPU 完了証跡にはしない。

```sh
.build/debug/nape-gesture sample-cpu --pid <nape-gesture PID> --duration 30 --interval 1 --mode idle --json --assert-baseline
```

`sample-cpu` は `measurementKind: "processCpuSampling"`、`includesEventTapAndPosting: false` の測定であり、CPU 使用率以外の入力遅延や画面反映は証明しない。
