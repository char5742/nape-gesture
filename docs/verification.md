...
.build/debug/nape-gesture analyze-log generated-space-right.jsonl --json --assert-generated-scroll-log --expected-direction positive-x --expected-normal-events 30 --expected-momentum-events 8 --expected-normal-x-total 1200 --expected-phase-mode auto
...
.build/debug/nape-gesture system-test run --scenario horizontal-scroll --post-to-pid <Reference Target App PID>
...
`analyze-log --assert-generated-scroll-log` は期待値の省略を許可しない。上の space-right 契約では通常30件、momentum changed 8件、momentum ended-zero 1件の計39件、正 X 方向、通常区間の point X 合計1200、per-step point量と量子化後scroll合計、`isContinuous == 1`、timestamp 厳密増加を終了コードで固定する。非ゼロ record の方向とpoint/scroll丸め量を照合するため、正負混在、合計相殺、逆方向、量不足、point/scroll 不一致を成功扱いしない。
`--expected-phase-mode auto` は通常1件なら `changed`、2件以上なら `began, changed*, ended` を要求する。momentum 0件なら終了recordなし、1件以上なら `changed+, ended-zero` を続ける。サブ1 pointの整数deltaゼロと `momentum-decay=0` のゼロchangedは生成器どおり受理する。phase なし、未知 phase、同一 record 重複、同一 timestamp、順序逆転、ended 後 tail、`isContinuous=2` は失敗する。`generate-scroll --phase began` などの明示 override は assertion 対象外であり、`--expected-phase-mode` に `auto` 以外を渡すと非ゼロ終了する。
`scrollDeltaX` / `scrollDeltaY` 合計が `Int64` を超える外部ログでも解析 process はtrapせず、表示集計を飽和させたうえで生成スクロール assertion がoverflowを非ゼロ終了にします。
`Fixtures/generated-scroll-auto-valid.jsonl` は同じ39件契約の成功 fixture である。completion collector は実際のgeneratorで1 step、momentumなし、サブ1 point、`momentum-decay=0` も成功させる。`Fixtures/generated-scroll-auto-*.jsonl` のその他のファイルは、切り詰め、途中欠落、方向・量・phase・timestamp・重複異常の expected failure を固定する。`Fixtures/sample-generated-scroll-log.jsonl` は `compare-log` 専用であり、この assertion では expected failure とする。
`systemTestScenario` / `sequenceIndex` は System Behavior Test 専用メタ情報であり、`generate-scroll` ログに混在していた場合は取り違えとして扱う。
...
`system-test run --dry-run --log-json` は、実イベントを投稿せず、System Behavior Test が生成する予定のスクロールまたはショートカットイベントを同じ JSON Lines 形式で保存する。dry-run レコードには `systemTestScenario` と `sequenceIndex` が入り、`analyze-log --json --assert-system-scenario <name>` でシナリオ名と順序も確認する。Spaces のスクロール系シナリオは `compare-log` の候補ログとして使い、`horizontal-scroll` は通常の横スクロール割り当ての scrollWheel イベント列として `analyze-log --json --assert-system-scenario horizontal-scroll` で水平 delta、phase、precise/continuous、`momentumPhase == 0` を確認する。Mission Control やページ戻る/進むなどの離散シナリオは `analyze-log --json --assert-system-scenario <name>` で keyDown / keyUp、keyCode、余計な modifier を含まない exact modifier flags を確認する。
`kill-switch` の単独シナリオも、実投稿時は他の未マーク入力シナリオと同じイベント列を使い、`keyDown` / `keyUp` の間隔を `interval` で明示する。ゼロ間隔の合成ショートカットは daemon の停止ログ確認に使わない。
Issue #10 のページ戻る、進む、ズーム、横スクロールは Safari / 対応アプリでの画面挙動確認が残るため、`page-back`、`page-forward`、`zoom-in`、`zoom-out`、`horizontal-scroll` の dry-run と `analyze-log --assert-system-scenario` は実機前証跡であり、完了扱いにはしない。
`space-left`、`space-right`、`horizontal-scroll` は `system-test run --scenario <name> --post-to-pid <Reference Target App PID>` で Reference Target App へ直接投稿し、ready file の `diagnostics.appIsActive/windowIsKey/windowIsMain/firstResponderIsCaptureView/focusInsideCaptureView` と `analyze-target-log --json --assert-has-generated-foreground-capture` で前面 AppKit 受信を確認できる。これは OS の画面遷移を避けるための無人 target 証跡であり、`--target finder` / `--target safari` と同時に使わない。
`mission-control`、`page-back`、`page-forward`、`zoom-in`、`zoom-out` の key shortcut 系は PID 直接投稿を採用しない。2026-07-10 の実験では target log に届かなかったため、dry-run の keyCode / modifier 検証と、実アプリ前面時の画面挙動検証を分けて保存する。
...
ready file の `diagnostics` には `appIsActive`、`windowIsKey`、`windowIsMain`、`firstResponderIsCaptureView`、`focusInsideCaptureView` などが入る。runtime event script と PID target 証跡はこれらが成立しない target log を採用しない。
...
保存した AppKit 受信ログは `analyze-target-log <path>` で集計し、`scrollWheel`、`swipe`、`magnify`、`rotate`、phase、momentumPhase、precise scroll の有無を確認する。生成 foreground スクロールは `canonicalGeneratedForegroundCapture*` の値で `sendEvent`、`localMonitor`、`captureView` の二重記録を排除して確認する。同じ NSEvent は capture source ごとに location の座標系が異なるため、canonical fingerprint は location ではなく timestamp、種別、delta、phase などで照合する。
`--assert-generated-foreground-scroll-x-positive` と `--assert-generated-foreground-scroll-x-negative` は X 合計の符号だけでなく、重複排除後の正負イベント数を検査する。期待方向と逆の値が1件でもあれば失敗し、phase 終了用の `scrollingDeltaX == 0` は許容する。
Issue #6 / #12 の最終実測へ進む前に、まず `analyze-target-log <path> --assert-no-leaks` で target log を機械判定する。
`gesture-drag` / `gesture-wheel` のように Nape Gesture 生成イベントが AppKit に届くこと自体が成功条件のシナリオでは、空ログや未成立ログを成功扱いしないため `--assert-has-generated-foreground-capture` も併用する。
...
`system-test run --post-to-pid <pid>` は対応シナリオの Reference Target App foreground 受信を切り分ける診断として使う。これだけで Issue #6 / #9 / #10 / #12 を完了扱いにせず、daemon 経路、`.cghidEventTap` 経路、対象アプリの画面挙動、実デバイス入力の証跡と分けて採否する。
...
- `gesture-drag`: `analyze-target-log --json --assert-no-leaks --assert-has-generated-foreground-capture` でジェスチャードラッグ中の元入力漏れがなく、生成イベントが前面 AppKit window に届くことを確認する
- `gesture-wheel`: `analyze-target-log --json --assert-no-leaks --assert-has-generated-foreground-capture` でジェスチャーホイール中の元入力漏れがなく、生成イベントが前面 AppKit window に届くことを確認する
...
- `gesture-wheel-then-kill-switch`: dry-run は `analyze-log --json --assert-kill-switch-shortcut --assert-gesture-before-kill-switch`、実イベント投稿後は daemon log と `analyze-target-log --json --assert-no-leaks --assert-has-generated-foreground-capture` でジェスチャー中のキルスイッチ停止と activation button release 抑制を確認する
...
for scenario in gesture-drag gesture-wheel gesture-wheel-then-kill-switch; do
  ...
  .build/debug/nape-gesture analyze-target-log "$target_log" --json --assert-no-leaks --assert-has-generated-foreground-capture
done
```
...
ジェスチャー生成を期待するシナリオでは `--assert-has-generated-foreground-capture` を併用し、target log が空、`globalMonitor` だけ、または通常入力だけだった実行を成功扱いしない。
...
暴走停止シナリオの前後で `target --out <path> --duration <秒> --ready-file <path>` を保存し、キルスイッチ後に Nape Gesture 由来でない `scrollWheel`、ドラッグ、ボタン、キー入力が混ざっていないことを `analyze-target-log <path> --json --assert-no-leaks --assert-has-generated-foreground-capture` で確認する。ジェスチャー中停止では activation button release も pending release として抑制されることを確認する。
...
5. `target --out target-space-left.jsonl --duration 8 --ready-file target-space-left.ready.json --focus-capture-point` を開き、ready diagnostics が active/key/main/focus 済みであることを確認してから、Reference Target App を前面にしたまま `system-test run --scenario space-left --post-to-pid <Reference Target App PID>` を実行して AppKit 受信差分を保存し、`analyze-target-log target-space-left.jsonl --json --assert-has-generated-foreground-capture --assert-generated-foreground-scroll-x-negative --assert-generated-foreground-scroll-events-at-least <受信下限> --assert-generated-foreground-scroll-abs-x-at-least <amount>` で集計する。AppKit は scrollWheel を合成することがあるため、受信下限は `steps` と同一視しない
