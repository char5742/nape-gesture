...
2026-07-09 の main `f43e217` では、`artifacts/completion/2026-07-09/gui-computer-use-main-f43e217/summary.md` に computer-use による `.build/NapeGesture.app` の設定ウィンドウ表示、主要設定項目、`保存して再起動` 押下と設定ファイル更新、通常アプリメニューの権限導線を保存している。
この証跡は GUI 操作の前進として扱うが、`doctor` は `accessibility.missing` と `targetDevice.notFound` を報告しているため、実イベント投稿、TCC 許可済み runtime、Nape Pro 実機操作の完成証跡にはしない。
...
| 通常 GUI 化でステータスメニューが消える | 開始、停止、緊急停止、権限確認をメニューバーから操作できない | Dock 表示対応時に `NSStatusItem` の作成や menu refresh が失われている | `.build/NapeGesture.app/Contents/MacOS/nape-gesture gui-smoke --config /tmp/nape-gui-smoke.config.json --json --assert` で `statusItemTitle: "NG"` と status menu の状態、開始、緊急停止、停止、設定、権限確認を確認する。実メニューバー上の表示やクリック操作の切り分けが必要な場合だけ computer-use で確認する |
