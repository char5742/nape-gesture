...
.build/debug/nape-gesture analyze-log generated-space-right.jsonl --json --assert-generated-scroll-log
...
`analyze-log --assert-generated-scroll-log` は、生成スクロールログが Nape Gesture 生成 `scrollWheel` だけで構成され、continuous/precise、timestamp 単調、通常 scroll phase と momentum phase の分離、momentum のゼロ delta 終了を満たすことを終了コードで固定する。
