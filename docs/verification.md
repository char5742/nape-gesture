...
`analyze-log --assert-generated-scroll-log` は期待値の省略を許可しない。上の space-right 契約では通常30件、非ゼロ momentum changed 8件、momentum ended-zero 1件の計39件、正 X 方向、通常区間の point/scroll X 合計1200、`isContinuous == 1`、timestamp 厳密増加を終了コードで固定する。全非ゼロ record で point/scroll X の符号、期待方向、生成時の丸め量を照合するため、正負混在、合計相殺、逆方向、量不足、point/scroll 不一致を成功扱いしない。
`--expected-phase-mode auto` は通常 `began, changed*, ended` の直後に momentum `changed+, ended-zero` が続く exact 状態列を要求する。phase なし、未知 phase、同一 record 重複、同一 timestamp、順序逆転、changed-zero 終端、ended 後 tail、`isContinuous=2` は失敗する。`generate-scroll --phase began` などの明示 override は assertion 対象外であり、`--expected-phase-mode` に `auto` 以外を渡すと非ゼロ終了する。
`Fixtures/generated-scroll-auto-valid.jsonl` は同じ39件契約の成功 fixture である。`Fixtures/generated-scroll-auto-*.jsonl` のその他のファイルは、切り詰め、途中欠落、方向・量・phase・timestamp・重複異常の expected failure を固定する。`Fixtures/sample-generated-scroll-log.jsonl` は `compare-log` 専用であり、この assertion では expected failure とする。
