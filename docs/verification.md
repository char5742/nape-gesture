...
2026-07-11に[ADR-0036](adr/0036-emulate-trackpad-driver-output-events.md)を採択し、製品gesture出力をtrackpad driver上位出力相当のscroll / gesture、DockSwipe、NavigationSwipe、magnification eventへ統一した。AX scrollbar、対象PID配送、application別分岐、keyboard shortcutによるgesture代替は完成形に含めない。
...
25F80のproduct capabilityは`scroll`、`DockSwipe`、`NavigationSwipe`、`magnification`をsupportedにする。`scroll`は確定contractと自前計測model、残る3 familyは候補raw fieldによる試用実装である。`doctor --json`の`supportedFamilies`が4 family、`missingRequiredFamilies`が空であることを確認する。未知OS buildやcontract/model不一致ではevent tap開始前に停止する。試用実装を#125、方向・係数調整、Nape Pro実機比較、公証の完了とは扱わない。
...
- Spaces / Mission Controlがprogressとphaseを持つDockSwipe event系列になっている
- page navigationがNavigationSwipe、zoomがmagnification / zoom event系列になっている
...
- 25F80 fixtureで確定したscroll / momentumと、#125の追加物理capture後に確定するNavigationSwipe / magnification / DockSwipe contractの差分report
...
product output testsでは、縦横scroll、`type 22 -> type 29 envelope -> type 29 companion`、scroll `began / changed / ended`、momentum `began / continued / ended`、line `0`とfixed-point / point `+0.0`のterminal、input / momentum active cancel、session順序、batch全件作成前の無投稿、投稿前raw field 39 / 40=`0`、direct post traceの`systemWide`、contract / model改変時のfail closed、不正な明示環境変数pathからfallbackしないことを検査する。
...
`system-test run --dry-run --log-json`は、実eventを投稿せず、移行前System Behavior Testの単純scrollまたはshortcut予定列を既存JSON Lines schemaで保存する。`systemTestScenario`と`sequenceIndex`のassertionは入力安全性と旧baselineの退行検出には残すが、trackpad driver上位出力contractやIssue #9 / #10のcompletion gateには使わない。
...
Issue #10のページ戻る、進む、ズーム、横スクロールに対する旧`page-back`、`page-forward`、`zoom-in`、`zoom-out`、`horizontal-scroll` dry-runは比較baselineに限定する。新しいNavigationSwipe、magnification、trackpad horizontal scrollの実機前証跡はraw contract fixtureと専用analyzerで作り直す。
...
完成形では、Spaces / Mission Control専用のkeyboard shortcutやapplication分岐を持たない。純正trackpad driver上位出力相当のDockSwipe event系列をsystem-wideに送り、macOSの標準gesture処理が連続transitionを行うことを確認する。
