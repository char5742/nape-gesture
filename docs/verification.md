...
この文書は、button 3 / 4 / 5ごとに選択した上位`GestureClass`へ接続する製品runtimeの検証手順を定義する。完成状態の正本は[完成判定チェックリスト](completion-checklist.md)、製品モデルの正本は[ADR-0049](adr/0049-fixed-button-to-gesture-class-input.md)とする。

## 検証対象

| GestureClass | 期待ProductOutput | `systemGestureSensitivity` |
| --- | --- | --- |
| 2本指scroll / swipe相当 | type 22 scrollと必要なtype 29 companion lifecycle | 適用しない |
| 3本指system swipe相当 | type 30 `DockSwipe`、motion 1 / 2 | 適用する |
| 4本指system pinch相当 | type 30 `DockSwipe`、motion 4 | 適用する |
...
button 3 / 4 / 5のそれぞれで3 classから1つを選択し、同じclassの重複割り当てを許可する。無効・未割り当て、方向別binding、button別・方向別・application別の感度設定を持たない。GUIの「システムジェスチャー感度」は物理button番号ではなく、選択された3本指 / 4本指classへ共通適用し、2本指classには適用しない。内部でclass固有adapterを選ぶことは、結果別routingではなく上位event contractのencodingである。

button 3 / 4 / 5のいずれも未押下の場合は、通常mouse入力をそのまま通す。

## 現在の確認状態

button割り当てを含む製品runtimeは同じ署名済みRelease候補へ接続し、全9対応、全27設定round-trip、GUI保存・再起動後復元まで機械検証済みである。物理受入対象の経路は次のとおり。
...
固定された既定割り当ての旧binaryでは、Nape Proの3 class合計23 session、5473 generated event、作成失敗0件、欠落投稿0件、全sessionのsingle terminal、終了後の通常mouse復帰まで受入済みである。Space切替、Mission Control、DockSwipe motion 4のsystem control遷移も確認済みである。この証跡は、変更可能な割り当て、保存、復元、重複、class基準の感度適用を証明しない。

次は別の未完了gateとして扱う。

- 既定button以外から各classを起動し、重複割り当てでも同じclass contractへ到達するNape Pro物理受入
...
button down時にそのbuttonの保存済み割り当てを解決し、releaseまたはcancelまでsource buttonと選択済みGestureClassをsessionへ固定する。進行中の設定変更や別button追加でclass、session ID、adapterを切り替えない。
...
- 3 buttonの割り当てを`gesture.buttonAssignments`へ保存し、27通りの割り当て組み合わせをround-tripする
- 割り当てがない固定モデルの設定へ既定値を補い、null、欠落key、未知class、無効・未割り当て値をcanonical形式として受け入れない
- 旧mode、`dragSensitivity`、`wheelSensitivity`、dead zone、application設定をcanonical形式から除去し、旧modeを現行割り当てへ、旧感度を`systemGestureSensitivity`へ推測移行しない
- `gesture.systemGestureSensitivity`がない旧設定には1.0を補い、既存値は0.25から2.0の範囲を検証する
- GUIに3つのbutton selectorがあり、それぞれ3 classだけを表示して重複選択を許可し、無効項目を持たない
- GUI sliderが25%から200%、既定100%で、button別・方向別・application別の感度controlを持たない
...
GUI smokeでは3 selectorの選択肢数、既定値、重複選択、割り当てと感度のdirty / revert、共通感度sliderと現在値表示、通常mouse通過説明、runtime状態、toolbar、詳細条件の開閉、pane切替、control数、非resizable windowをAppKit上で検証する。実際のLaunchServices起動ではAX treeと画面を確認し、一時割り当てを編集・保存・再起動してdisk反映とruntime反映を検証する。
...
- 9通りのbutton-class対応をそれぞれ開始できる
- 2つ以上のbuttonへ同じclassを割り当てても、各source buttonで独立したsessionとして開始・終了できる
- 無効または未割り当て状態を作れない
- 3本指 / 4本指classはどのbuttonから開始しても共通感度が適用され、2本指classにはどのbuttonから開始しても適用されない
...
- buttonとGestureClassの対応を変更不能とする
- 無効・未割り当て、button別感度、方向別またはapplication別割り当てを追加する
