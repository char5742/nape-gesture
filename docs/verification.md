...
常駐 CPU 使用率は、対象 runtime executable を直接起動したコマンドの `$!` と、その executable の絶対パスを `sample-cpu --expected-executable ... --json --assert-baseline` に渡して保存する。GUI runtime は `.build/NapeGesture.app/Contents/MacOS/nape-gesture` 自身を対象にする。`pgrep`、`open`、`swift run` は対象 executable 自身の PID を一意に確定できないため使わない。再現可能な起動・終了手順は `docs/performance-baseline.md` の「実機 CPU 測定手順」を正とする。
completion evidence の短時間 smoke や `/bin/sleep` PID への実行は、コマンド形式と同一性検査の退行確認であり、常駐 CPU 完了証跡にはしない。

```sh
.build/debug/nape-gesture sample-cpu --pid <直接起動で得た PID> --expected-executable <対象実行ファイルの絶対パス> --duration 30 --interval 1 --mode idle --json --assert-baseline
```

`sample-cpu` は `proc_pidpath` で解決した実行ファイル、`proc_pidinfo` の開始時刻トークン、`task_info(TASK_AUDIT_TOKEN)` と libbsm で得た `(pid, pidversion)` を開始時・各 sample の前後で照合する。`ProcessInfo.systemUptime` の deadline 到達後まで採取し、`actualDurationSeconds >= requestedDurationSeconds`、`requestedDurationReached == true`、初回と最終 sample の存在を確認する。既存の `schemaVersion: 1` とキーに加えて report と各 sample の `processIDVersion` を確認し、`expectedExecutablePath`、`resolvedExecutablePath`、`executableIdentityMatched`、`processStartToken`、`processIdentityStable`、`baseline.passed` と合わせて duration 未達、実行主体不一致、途中変化を不合格にする。開始トークン変化は PID 再利用、pidversion 変化は同一 path を含む `exec` / `posix_spawn`、pidversion が同じ path 変化は実行ファイル変化として区別する。audit token 取得不能も fail closed とし、`processCommand` は診断表示用で合格条件には使わない。途中終了や target 変化の回帰では、1件目の有効 sample と前後 identity 照合後に `completedSampleCount: 1` を含めて公開される `--ready-file <path>` の完全 JSON を待ってから trigger し、失敗 report にも1件以上の sample があることを確認する。ready は一時 sibling から `renamex_np(..., RENAME_EXCL)` で排他的に公開され、16並列競合で成功が1件だけ、既存ファイルの上書きと一時ファイル残留がないことを確認する。`--out` と ready path は component を左から処理し、存在する symlink をその地点で展開してから後続の `..` を適用する。`alias/link/../report.json` と symlink 後に非存在 component / `..` を含む相対 path、root、case-only path を同一実体として拒否する。
`measurementKind: "processCpuSampling"`、`includesEventTapAndPosting: false` の測定であり、CPU 使用率以外の入力遅延や画面反映は証明しない。
