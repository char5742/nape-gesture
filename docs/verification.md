...
`space-left`、`space-right`、`horizontal-scroll` は `system-test run --scenario <name> --post-to-pid <Reference Target App PID>` で Reference Target App へ直接投稿し、`analyze-target-log --json --assert-has-generated-event --assert-has-foreground-capture` で前面 AppKit 受信を確認できる。これは OS の画面遷移を避けるための無人 target 証跡であり、`--target finder` / `--target safari` と同時に使わない。
...
5. `target --out target-space-left.jsonl --duration 8 --ready-file target-space-left.ready.json` を開き、Reference Target App を前面にしたまま `system-test run --scenario space-left --post-to-pid <Reference Target App PID>` を実行して AppKit 受信差分を保存し、`analyze-target-log target-space-left.jsonl --json --assert-has-generated-event --assert-has-foreground-capture` で集計する
