...
Issue #10 のページ戻る、進む、ズーム、縦横スクロールは Safari / 対応アプリでの画面挙動確認が必要である。`page-back`、`page-forward`、`zoom-in`、`zoom-out`、`horizontal-scroll` の dry-run と `analyze-log --assert-current-uptime --assert-system-scenario` は実操作前証跡であり、単独では完了扱いにしない。[Safari scroll 配送比較](safari-scroll-delivery-verification.md) と `docs/fixtures/safari-scroll-probe/contract.json` の5 assertionで、unlabeled generic overflow の fail-closed、縦のみ frame への微小横成分混在、通常の長い article、`AXWebArea` 自身の scrollbar、Computer Use の wheel handler を分けて比較する。時刻修正前に得た generic / frame / article / top-level の値は selector 比較専用であり、ADR-0037 統合 head の完成証跡には流用しない。contract の静的整合は `python3 scripts/check-safari-scroll-probe-contract.py` で completion に固定するが、CGEvent logとSafari画面挙動の代替にはしない。AX scrollbar set は wheel handler を発火せず、公開 AX tree が generic container を完全に省略する場合の通常 wheel 同等 semantics は未成立である。
...
ADR-0037 はこの branch へ統合済みである。最終 Safari / runtime 証跡は同じ統合 commit と TCC 許可済み `.app` identity を固定し、contract の5 assertion、通常 async、PID固定 sync、端到達、Computer Use の通常 wheel、生成 CGEvent log の `--assert-current-uptime`、画面挙動、runtime performance completion を再取得する。
