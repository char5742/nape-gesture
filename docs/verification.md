...
.build/debug/nape-gesture system-test readiness --json --assert > system-test-readiness.json
.build/debug/nape-gesture system-test readiness --markdown --assert > system-test-readiness.md
...
`system-test readiness --json --assert` は、全 scenario の exact Issue 集合、expected target、event shape、前段の機械証跡コマンド、runtime / target 証跡対象、画面挙動実測待ち、`need:human` 境界、completion state を同じ JSON schema で保存する。Markdown も runtime / target 証跡を省略しない。
`system-test matrix` は同じ出力の別名であり、completion evidence では alias の退行も確認する。
この assertion は readiness 定義の欠落を検出するだけで、Spaces / Mission Control や Safari の画面が動いた証跡ではない。
...
System Behavior Test の前段証跡と画面挙動待ちの一覧は [System Behavior Test matrix](system-behavior-matrix.md) を正とする。
