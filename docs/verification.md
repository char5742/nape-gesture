...
`generate-scroll`と`system-test`の実投稿およびdry-run logは、`MonotonicEventClock`のmacOS起動後時刻を使う。`nape-gesture-diagnostic-output-tests`は、現在boot内のtimestampをCGEventへ保持し、負値、非有限値、Unix epoch、未来boot値をevent作成失敗にすることを投稿なしで検査する。`sh scripts/check-diagnostic-event-time.sh`は対象3経路へのwall clock再混入を失敗にする。
