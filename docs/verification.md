...
`sample-cpu` は `proc_pidpath` で解決した実行ファイルと `proc_pidinfo` の開始時刻トークンを開始時・各 sample の前後で照合する。`expectedExecutablePath`、`resolvedExecutablePath`、`executableIdentityMatched`、`processStartToken`、`processIdentityStable` と `baseline.passed` を確認し、実行主体不一致や途中変化を CPU 基準内でも不合格にする。`processCommand` は診断表示用であり、合格条件ではない。
