...
| `.app` が Dock に出ない | 起動しても通常 GUI アプリとして見えず、設定画面を見つけにくい | `LSUIElement` が Boolean `false` 以外に戻っている、activation policy が `.accessory` に戻っている、または画面取得権限の境界で観測に失敗している | `verify-bundle` と `scripts/test-verify-bundle.sh` で exact Boolean 型の `LSUIElement=false` を確認し、`app` 起動経路が `.regular` であることを確認する。画面観測では computer-use の app state と System Events の Dock process で `NapeGesture` item を確認する |
