...
Issue #10 のページ戻る、進む、ズーム、横スクロールは Safari / 対応アプリでの画面挙動確認が残るため、`page-back`、`page-forward`、`zoom-in`、`zoom-out`、`horizontal-scroll` の dry-run と `analyze-log` は実機前証跡であり、完了扱いにはしない。
