...
Issue #10 のページ戻る、進む、ズーム、横スクロールは Safari / 対応アプリでの画面挙動確認が必要である。2026-07-10 時点で、権限付与済みの `.build/NapeGesture.app/Contents/MacOS/nape-gesture` から Safari へ投稿し、`page-back` / `page-forward` / `zoom-in` / `zoom-out` の画面挙動は確認済みである。`horizontal-scroll` は通常スクロール経路をポインタ直下 window owner PID へ送る実装に切り替え、Reference Target App の `sendEvent` / `localMonitor` 受信は確認した。ただし Safari Web content は実カーソルが main content 上にある状態でも CGEvent 合成 scrollWheel で `scrollX` / `scrollY` が変化せず、computer-use のネイティブ scroll では `scrollY=0` から `scrollY=816` へ変化したため、Safari 画面反映は未解決として分けて扱う。詳細は [ADR-0035](adr/0035-scroll-event-target-process-routing.md) を参照する。
