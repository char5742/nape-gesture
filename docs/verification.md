...
Issue #10 のページ戻る、進む、ズーム、横スクロールは Safari / 対応アプリでの画面挙動確認が必要である。2026-07-10 時点で、権限付与済みの `.build/NapeGesture.app/Contents/MacOS/nape-gesture` から Safari へ投稿し、`page-back` / `page-forward` / `zoom-in` / `zoom-out` の画面挙動は確認済みである。`horizontal-scroll` は通常スクロール経路をポインタ直下 window owner PID へ送る実装に切り替え、Reference Target App の `sendEvent` / `localMonitor` 受信を確認したうえで、Safari Web content では CGEvent 投稿ではなく `AXWebArea` horizontal scrollbar 経路へ切り替える。Safari では CGEvent 合成 scrollWheel 単体では `scrollX` / `scrollY` が変化しないが、document の `maxX=3544` を確認した検証ページで `system-test run --scenario horizontal-scroll --target safari --amount 1600 --steps 32` が `scrollX=0` から `scrollX=1272` へ変化し、`generate-scroll --mode horizontal --x -1600 --steps 32` が `scrollX=1272` から `scrollX=0` へ戻ることを computer-use で確認した。詳細は [ADR-0035](adr/0035-scroll-event-target-process-routing.md) を参照する。
...
target log を検証する場合、`system-test run` には `--target finder` / `--target safari` を付けない。`--target` を指定すると Finder または Safari が前面化するため、Reference Target App の AppKit 受信ログではなく、`log` や画面挙動の検証として扱う。通常スクロール系の `.free` / `.horizontal` はポインタ直下 window owner PID へ `postToPid` するため、target log では実カーソルが対象 window 上にあることを `CGEvent(source: nil)?.location`、CGWindow bounds、必要に応じて `AXUIElementCopyElementAtPosition` で確認する。
...
`system-test run --post-to-pid <pid>` は未マーク通常入力の Reference Target App sink 診断専用である。通常スクロールの生成イベントは [ADR-0035](adr/0035-scroll-event-target-process-routing.md) に従い、ポインタ直下 window owner PID へ送る。
