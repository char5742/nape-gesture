...
target_pid=$!
until test -f "$ready_file"; do sleep 0.1; done
# ここで対象デバイス操作、またはアクセシビリティ許可済みの run / system-test を別プロセスで実行する。
wait "$target_pid"
.build/debug/nape-gesture analyze-target-log "$target_log" --json --assert-no-leaks
```
...
5. `target --out target-space-left.jsonl --duration 8 --ready-file target-space-left.ready.json` を開き、基準ウィンドウ上で純正トラックパッド、Nape Pro、生成イベントの AppKit 受信差分を保存し、`analyze-target-log target-space-left.jsonl --json --assert-no-leaks` で集計する
