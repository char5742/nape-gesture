...
- trackpad scrollでcontinuous scroll eventとcompanion gesture eventが同一timestamp系列になっている
...
比較ではevent type、subtype、公開scroll delta、fixed-point delta、point delta、gesture delta、phase、momentum、continuous / inverted flag、timestamp、event順序を確認する。通常scrollのbegin / change / end / cancelとmomentum begin / continue / endが混ざる場合、companion gesture eventが欠落する場合、同一frameのtimestamp対応が崩れる場合はcontract不一致として修正する。
