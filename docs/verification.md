...
Issue #10 のページ戻る、進む、ズーム、縦横スクロールは Safari / 対応アプリでの画面挙動確認が必要である。2026-07-10 時点で、権限付与済みの `.build/NapeGesture.app/Contents/MacOS/nape-gesture` から Safari へ投稿し、`page-back` / `page-forward` / `zoom-in` / `zoom-out` と top-level 縦横スクロールを確認済みである。ただし top-level の変化だけを Web scroll 完成証跡にしない。[Safari scroll 配送比較](safari-scroll-delivery-verification.md) の repo fixture で、generic overflow の fail-closed、AX accessible frame の inner-only 更新、端到達、Computer Use の wheel handler、CGEvent variant を別々に確認する。AX scrollbar set は wheel handler を発火せず、公開 AX tree が generic container を省略する場合の通常 wheel 同等 semantics は未成立である。
runtime では AX fallback、同じ入力の CGEvent fallback、離散ショートカットを serial queue に載せ、AX request がない Spaces は同期直送する。daemon 起動時と activation button 押下時に水平 / 垂直 Web capability を全体 40ms で prewarm し、1 call 20ms、実配送探索 120ms、async 最大待ち 160ms、window target cache 500ms、normalized value cache 250ms とする。AX target は application hit-test から毎 step 再解決し、container identity を value cache key に含める。window 全体の descending 探索と non-Web miss cache は使わない。runtime 性能ログは schema 2 の provisional と queue 内 completion を command ID で解決し、completion がない deferred record は baseline を通さない。`pr101-ax-delivery-final6` の p95 0.10〜1.76ms は AppKit 受信、nested target、Safari 画面反映、wheel handler を含まない。
