...
Issue #10 のページ戻る、進む、ズーム、縦横スクロールは Safari / 対応アプリでの画面挙動確認が必要である。2026-07-10 時点で、権限付与済みの `.build/NapeGesture.app/Contents/MacOS/nape-gesture` から Safari へ投稿し、`page-back` / `page-forward` / `zoom-in` / `zoom-out` の画面挙動は確認済みである。通常スクロール経路はポインタ直下 window owner PID へ送り、Safari Web content では `AXWebArea` scrollbar 経路へ切り替える。実カーソル下が Web content で通常 window owner が Safari になる状態では、PID override なしの async 経路で横 `scrollX=0 -> 1609`、縦 `scrollY=0 -> 1675`、斜め負方向で `scrollX=0 / scrollY=0` への復帰を確認した。Codex window が対象判定を覆う状態では、Safari サイドバーを閉じて実カーソル下を Web content にし、`--post-to-pid <Safari PID>` で診断対象を固定して再検証した。`maxX=2876 / maxY=3350` のページで横 32 step は `scrollX=0 -> 1438`、縦 32 step は `scrollY=0 -> 1675`、斜め負方向 32 step は両軸 `0` へ復帰し、final6 `.app` の wall time は各 0.37〜0.38 秒だった。詳細は [ADR-0036](adr/0036-scroll-event-target-process-routing.md) を参照する。
runtime では AX fallback、同じ入力の CGEvent fallback、離散ショートカットを serial queue に載せ、AX request がない Spaces は同期直送する。daemon 起動時と activation button 押下時に水平 / 垂直 Web capability を全体 40ms で prewarm し、application 固定 hit-test、1 call 20ms、実配送探索 120ms、async 最大待ち 160ms、window bounds 内の成功 target cache 500ms、AX target/value cache 250ms、完了済み non-Web 判定 cache 500ms とする。window target 不在と AX timeout は miss cache しない。Spaces は不要な pointer window 探索を行わない。runtime 性能ログは schema 2 の provisional と queue 内の実配送 completion を command ID で解決し、completion がない deferred record は baseline を通さない。`artifacts/completion/2026-07-10/pr101-ax-delivery-final6` では Spaces / gesture-wheel / gesture-wheel-then-kill-switch の runtime performance baseline が成功し、`deferredDeliveryRecordCount=0`、tap callback から実配送完了までの p95 は 0.10〜1.76ms だった。この値は AppKit 受信、Safari の画面反映、32 step の wall timeを含まない。
Codex などの自動化ホスト window が Quartz 上の実ポインタ位置を覆う場合は、`generate-scroll --post-to-pid <Safari PID>` で診断対象だけを固定できる。この override は画面証跡取得用であり、通常 runtime のポインタ直下 window owner 選択を変更しない。
