...
`--assert-valid-window` は、`--target-stable-id` がない場合、解析対象イベントがない場合、互換 HID 候補なし、非互換 HID 近傍、対象外互換 HID 近傍、採用 HID デバイスが対象 stableID と一致しない場合、または associationWindow 外のイベントがある場合に非ゼロ終了する。移動/ドラッグは HID Generic Desktop X/Y、スクロールは Generic Desktop Wheel または Consumer AC Pan、ボタンは `buttonNumber + 1` の HID Button usage とだけ関連付ける。実機ログ取得後の採否は、人間判断ではなく `analyze-association --json --assert-valid-window --target-stable-id <ID>` の終了コード、`matches.expectedHIDUsages`、`matches.nearestIncompatibleHID`、`matches.nearestTargetMismatchHID`、`matchedHIDDeviceIDs`、`matches` の時刻差で行う。
