...
Issue #10 のページ戻る、進む、ズーム、横スクロールは Safari / 対応アプリでの画面挙動確認が必要である。2026-07-10 時点で、権限付与済みの `.build/NapeGesture.app/Contents/MacOS/nape-gesture` から Safari へ投稿し、`page-back` / `page-forward` / `zoom-in` / `zoom-out` の画面挙動は確認済みである。`horizontal-scroll` は Safari Web content の `scroll-x` が変化しないため、未解決として分けて扱う。
