...
Nape Gestureの完成形は、特定ボタン押下中のmouse操作を、Mac Mouse Fixの`Scroll & Navigate`や`Spaces & Mission Control`と同等にmacOSへ認識させることである。AX scrollbar、対象PIDへのevent配送、keyboard shortcutによるgesture代替は、前面applicationごとの分岐と不完全な挙動を生むため採用しない。

Mac Mouse Fixの公式リポジトリを再確認したところ、通常配布物にDriverKit System Extensionはなく、input event tapとhelper内の出力moduleで構成されている。二本指相当の経路はcontinuous scroll eventと対応するgesture eventをphase / momentum付きで送り、Spaces / Mission Control相当の経路は連続progressとphaseを持つDockSwipe eventを送る。page navigationとmagnificationもtrackpad driverの上位出力に相当するgesture eventとして扱う。

この構成は目標挙動の参考にするが、Mac Mouse Fixのコード、field番号、定数、係数、状態遷移はコピーしない。同リポジトリは独自ライセンスであり、実装由来を混ぜない境界を維持する。

## 決定

- 入力側は既存のIOHID device識別、CGEvent tap、対象device association、gesture recognizerを基礎にする。
- 製品出力は次のtrackpad driver上位出力eventへ限定する。
  - trackpad scroll: continuous scroll eventと対応するscroll gesture eventの同一timestamp系列
...
- Mac Mouse Fix由来の名前、数値、header、実装断片はproduction code、fixture、testへ持ち込まない。
...
- [Mac Mouse Fix README](https://github.com/noah-nuebling/mac-mouse-fix/blob/29187456cb08c087d411beb7c08a0f4d52fd1c22/Readme.md)
- [Mac Mouse Fix gesture scroll出力](https://github.com/noah-nuebling/mac-mouse-fix/blob/29187456cb08c087d411beb7c08a0f4d52fd1c22/Helper/Core/Touch/GestureScrollSimulator.m)
- [Mac Mouse Fix二本指相当drag](https://github.com/noah-nuebling/mac-mouse-fix/blob/29187456cb08c087d411beb7c08a0f4d52fd1c22/Helper/Core/Drag/TwoFingerSwipe/ModifiedDragOutputTwoFingerSwipe.m)
- [Mac Mouse Fix三本指相当drag](https://github.com/noah-nuebling/mac-mouse-fix/blob/29187456cb08c087d411beb7c08a0f4d52fd1c22/Helper/Core/Drag/ThreeFingerSwipe/ModifiedDragOutputThreeFingerSwipe.m)
