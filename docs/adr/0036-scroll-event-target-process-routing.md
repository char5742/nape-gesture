...
- normalized value cache は PID / window / pointer に加えて、解決した container identity を key に含める。同一点で nested / outer target が変われば再利用しない。250ms 経過後は新しく解決した scrollbar の実値を読む。
- ancestor を `AXApplication` まで完全走査し、純粋 selector が `notHandled` と確定した結果だけを PID / window / exact pointer / requested axes を key に500ms保持する。`found`、ambiguous、情報不足、利用可能軸0の `blocked` では即破棄する。非Web上の連続入力でAX全探索を毎回直列化せず、Webの安全側 `blocked` をCGEvent fallbackへ変えない。
...
- `system-test` の連続scrollは、端到達または一時的な`noChange`で系列を打ち切らず全stepを実行する。CGEvent作成失敗は即時失敗とし、作成失敗がなく系列全体で1件以上配送できた場合だけ成功、全step 0件は配送不能として失敗にする。
...
- Issue #102 の時刻ドメインは ADR-0037 としてこの branch に統合済みである。PR #101 の最終 Safari / runtime 証跡は、単一候補commitとTCC許可済み`.app` identityを固定し、contractの6 assertion / 12 run、通常async、PID固定sync / async、frame端到達、Computer Useの通常wheel、生成CGEvent logのcurrent uptime、runtime performance completionを再取得する。runtime evaluatorのexit `0` / passを必要条件とし、PID固定診断やexit `2` / blockedを通常routing成功へ昇格させない。
