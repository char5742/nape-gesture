...
- Issue #102 の時刻ドメインは ADR-0037 としてこの branch に統合済みである。PR #101 の最終 Safari / runtime 証跡は、統合 commit と TCC 許可済み `.app` identity を固定し、contract の5 assertion、通常 async、PID 固定 sync、端到達、Computer Use の通常 wheel、生成 CGEvent log の current uptime、runtime performance completion を再取得する。
