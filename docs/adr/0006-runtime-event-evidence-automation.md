...
- `gesture-drag`、`gesture-wheel` は `--assert-no-leaks --assert-has-generated-foreground-capture` を使い、未マーク入力が前面アプリへ届いた場合、または Nape Gesture 生成イベントが foreground AppKit 経路へ届かなかった場合に失敗させる。
- `kill-switch` は生成イベントが届かないことも正常系になり得るため、`--assert-has-generated-foreground-capture` を使わない。`--assert-no-leaks` を使い、未マークキー入力が前面アプリへ届いた場合に失敗させる。
...
- 暴走中停止の前段証跡は `gesture-wheel-then-kill-switch` を使う。dry-run は `analyze-log --assert-kill-switch-shortcut --assert-gesture-before-kill-switch`、runtime event は daemon 停止ログと `analyze-target-log --assert-no-leaks --assert-has-generated-foreground-capture` で判定する。
