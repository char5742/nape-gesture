...
- SystemUIServer の Accessibility name に `NSStatusItem` が露出しない場合、それだけで status item 不在とは判定しない。status item `NG` と status menu の生成契約は AppKit `gui-smoke --json --assert` と source-level 契約で確認する。
