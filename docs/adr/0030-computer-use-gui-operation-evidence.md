...
Nape Gesture の完成判定には、通常 GUI アプリの起動、設定ウィンドウ、メニューバーのsystem symbol、System Settings の権限導線、TCC 反映後の再実行など、CLI だけでは確認しにくい macOS UI 操作が含まれる。
...
- GUI アプリの起動確認、設定ウィンドウ表示、メニューバーのsystem symbol操作、System Settings pane の表示確認、スクリーンショット取得は computer-use の対象にする。
...
- Terminal の `screencapture` が Screen Recording 権限境界で失敗した場合、それだけで製品不具合や GUI 証跡不足とは判定しない。computer-use の app state、Accessibility tree、System Events の process / Dock item 観測を優先する。
- SystemUIServer の Accessibility name に `NSStatusItem` が露出しない場合、それだけで status item 不在とは判定しない。status item symbol と status menu の生成契約は AppKit `gui-smoke --json --assert` と source-level 契約で確認する。
...
- Screen Recording や AX name 露出の環境差を、製品側の runtime / UI 契約と分けて扱える。
