...
- `gui-smoke --config <path> --json --assert` は、runtime を開始せずに `.app` 実行主体で AppKit 内の `.regular` activation policy、設定ウィンドウ、accessibility label付きstatus item symbol、通常アプリメニュー、status menu の生成契約を機械検査する。`--config` 未指定時は一時 config を使い、ユーザーの通常設定へ書き込まない。
...
- メニューバー常駐 UI は残るため、実行中の開始、停止、権限確認は従来どおり使える。
- 初回起動のウィンドウ表示と status item symbol の AppKit 契約は Info.plist、起動コード、`gui-smoke --config <path> --json --assert` で固定する。実メニューバー上のクリック操作が必要な場合は computer-use で確認する。SystemUIServer の Accessibility name に status item が露出しない場合でも、それだけで status item 不在とは判定しない。
