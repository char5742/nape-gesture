...
- completion evidence は `page-back`、`page-forward`、`zoom-in`、`zoom-out`、`horizontal-scroll` の `system-test run --dry-run --log-json` と `analyze-log --json --assert-system-scenario <name>` を保存する。dry-run JSON Lines には `systemTestScenario` と `sequenceIndex` を含め、取り違えや順序欠落を終了コードで検出する。
...
- ページ戻る / 進む / ズーム / 横スクロールの前段証跡が、シナリオ名と終了コードつきで completion evidence に継続して残る。
