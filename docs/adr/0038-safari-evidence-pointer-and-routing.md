...
- 通常routingの証跡では、投稿直前の frontmost appに加えて、`swift scripts/capture-pointer-window-stack.swift` でQuartzポインタを含むlayer 0 window stackの先頭owner名、PID、window number、boundsをJSON保存する。wall clockは補助情報とし、生成イベントとの照合には同時保存するsystem uptimeを使う。
- `--post-to-pid <Safari PID>` はAX target選択とsync/async queueの診断に限定する。PID固定成功を、通常のpointer-window owner選択成功として扱わない。
- Codex / ChatGPTなど自動化ホスト自身のwindowが通常routingを覆う場合、Computer Useからホスト自身を操作しない。まず同じCodex親プロセスのforeground shellで `scripts/set-codex-host-visibility.swift hide`、Safari前面化、window stack取得、候補`.app`実行、`activate`による復帰を1処理内で行う。復帰はshell trapでも保証し、この方法まで成立しない場合だけ`need:human`とする。
...
- manifest schema 2は候補commit、`.app` executable、正本contract、各artifactのSHA-256を固定する。artifactはrun専用directory外参照、正規化後の逸脱、run間path共有を禁止し、生成操作は実argv、native wheelはComputer Useのdirection/pagesを保存する。
- 通常routingの事前条件未成立は`executionStatus: precondition-blocked`、`invocation: null`、pointer/window証跡だけで表現する。操作後snapshotや成功exit codeを持つ`executed`と混在させず、exit `2`を完成扱いしない。
- frame端はprobe schema 2の`maxY`と`atEnd`を保存し、`atEnd == (y >= maxY)`を再計算する。`y=10000`後に端到達し、続く`y=800`でframe/outerが不変であることを別遷移として判定する。
- fixtureのCSS寸法と固定snapshotのずれをCIで見逃さないため、WebKitで実fixtureを描画し、初期`maxY`、実終端の`y == maxY` / `atEnd=true`、親iframeへの状態反映をcontract値と照合する。
- launchd経由の候補実行はTCC責任主体が変わり、同じ`.app` pathでもアクセシビリティ未許可になるため通常routing証跡に使わない。候補実行は権限確認済みCodex主体のforeground処理内に保つ。
