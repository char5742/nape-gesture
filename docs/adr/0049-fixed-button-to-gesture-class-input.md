...
- 3本指classはtype 30 / classifier 23、phase fields 132 / 134、IOHID DockSwipe motion 1 / 2へ変換する。progress / XY positionはsource delta / 600、終端XY velocityはsource velocity / 600とする。
- 4本指classは同じtype 30 / classifier 23とphase 1 / 2 / 4 / 8を使うが、IOHID DockSwipe motion 4へ変換する。progressはY優先のsigned source delta / 600、終端Z velocityは同じ符号規則のsource velocity / 600とする。

このため、同じsource系列でもgenerated event type、event count、field、phase、unit conversionはclassごとに異なり得る。3本指と4本指はevent family、motion、axis、符号規則が異なる一方、mouse source deltaとsource velocityの`/ 600`固定変換スケールは共有する。このencodingを各classの純正trackpad fixtureへ照合する。
