...
- `sample-cpu --pid <pid> --duration <秒> --interval <秒> --mode idle|active|recovery --json --assert-baseline` は、指定 PID の `%CPU` を周期サンプルし、常駐 CPU 使用率を終了コードで判定する。
...
- `scripts/collect-completion-evidence.sh` の `sample-cpu` はコマンド形式の smoke として扱い、日常利用主体の常駐 CPU 完了証跡には昇格しない。
...
- 常駐 CPU は、日常利用と同じ `.app` または実行ファイルの PID を指定した `sample-cpu` の JSON と終了コードで判定できる。
