...
- `--out` と `--ready-file` の衝突判定は、既存 ancestor の symlink を解決し、まだ存在しない leaf component を大文字小文字を区別せず比較する。case-sensitive volume でも安全側に倒し、case-only difference は同一ファイル候補として実行前に拒否する。
