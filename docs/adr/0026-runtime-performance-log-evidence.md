...
- `--ready-file` は1件目の有効 CPU sample を採取し、その前後の identity snapshot が一致した直後に公開する。ready JSON は `schemaVersion: 1`、`ready: true`、`completedSampleCount: 1`、PID、`processStartToken`、`processIDVersion`、`resolvedExecutablePath`、`timestampUnixSeconds` を含み、1件の有効 sample 完了だけを表して duration 到達や baseline 合格を表さない。
- ready JSON は完全な内容を一時 sibling に書き終えた後、macOS の `renamex_np(..., RENAME_EXCL)` で final path へ no-replace atomic rename する。並列競合では1プロセスだけが成功し、敗者は既存ファイルを上書きせず明示的に失敗し、一時ファイルを削除する。空または部分 JSON を final path へ先に公開しない。
- `--out` と `--ready-file` の衝突判定は path component を左から処理し、各地点で `lstat` により存在を確認した symlink を target へ展開してから、symlink より後ろの `..` を適用する。非存在 component は component stack に保持して後続の `..` で戻せるようにし、相対 path は現在の作業ディレクトリから、root より上の `..` は root として解決する。最後に大文字小文字を区別せず比較し、case-sensitive volume でも case-only difference を同一ファイル候補として実行前に拒否する。symlink loop や権限エラーで安全に解決できない場合も衝突として fail closed にする。
