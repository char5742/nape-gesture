...
- Issue #13 の実機作業前段として `recovery-readiness --json --assert` を追加する。この report は、`sleep-wake-runtime-retry`、`target-device-disconnect`、`target-device-reconnect`、`accessibility-permission-change`、`input-monitoring-permission-change`、`recoverable-runtime-failure`、`human-fix-required-failure` を列挙し、機械で固定済みの契約、機械 assertion、残る外部証跡、`need:human` 候補を構造化する。
- `recovery-readiness --assert` は、外部証跡が必要なシナリオを `completed` にしないことも検査する。これにより、スリープ、抜き差し、TCC 変更の実ログがない状態で Issue #13 を完了扱いにする退行を防ぐ。
- `scripts/collect-completion-evidence.sh` は `recovery-readiness/recovery-readiness.json` と `recovery-readiness/recovery-readiness.md` を保存する。
...
- `recovery-readiness` により、Issue #13 の各実機シナリオで何を machine readiness として扱い、何を人間作業または外部状態として残すかを PR ごとに再現できる。
