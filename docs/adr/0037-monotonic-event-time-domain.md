...
- `system-test run` は256 step、系列全体30秒を上限とする。256 step は AX 同期探索の1 event 120ms上限を含めても CGEvent の60秒 skew境界に余裕を残す。派生 delta、速度、全 timestamp、全 CGEvent を1件目の投稿前に検証し、scroll系列では最後の要素後に不要な sleep を行わない。JSON Lines は全件を encode してから stdout または `--out` を更新し、失敗時に部分ログや既存ファイルの置換を残さない。
