...
`physical-observations.json`は、純正trackpadの8 capture、candidate family、未取得境界をまとめた観測台帳である。
NavigationSwipe、magnification、DockSwipe、Mission Control / App Exposeの追加captureにより今後も更新されるため、この台帳全体のSHA-256を確定済みscroll / momentum実行契約として使うと、無関係な観測追加でも契約identityが変わる。
...
- 観測台帳の`partial`状態は維持する。専用fixtureの`confirmed`はscroll / momentumだけを指し、NavigationSwipe、magnification、DockSwipe、Mission Control / App Exposeを完成扱いにしない。
...
- candidate familyの追加観測と、確定済みscroll契約のidentityを独立して更新できる。
- 生成候補のmissing terminal、missing companion、未確定type 29、envelope、phase、field alias、OS build差分を同じCLI reportと終了codeで判定できる。
- analyzer合格だけでは製品runtimeをsupportedにしない。製品adapterとruntime registryへの登録はIssue #119 / #122の別gateで行う。
