# ADR-0047: 方向別bindingを廃止しボタンごとのtrackpad modeへ接続する

- 状態: 採択
- 日付: 2026-07-12

## 背景

従来の操作体系は、ジェスチャーボタン押下中の上下左右ドラッグを個別actionへ割り当て、確定方向に応じて出力familyを選択していた。この方式では、同じドラッグ中の方向転換が別actionへの切り替え候補となり、下方向だけscrollになるなど、トラックパッドの連続した2D入力と異なる挙動を生む。方向ロックと軸ずれキャンセルも、押下中のX / Y変化を連続的にmacOSへ渡すことを妨げる。

製品の目的はマウス移動を方向別actionへ割り当てることではない。button 3 / 4 / 5のそれぞれにmodeを割り当て、押下中の連続した2D入力を、開始方向によらずそのbuttonで選んだtrackpad gesture系列へ渡すことである。

## 決定

- 製品の操作体系は、button 3 / 4 / 5ごとにmodeを割り当てる。選択肢は通常mouseとして扱う`none`、`Scroll & Navigate`、`Spaces & Mission Control`、`Zoom`とする。
- 既定割り当てはbutton 3=`Scroll & Navigate`、button 4=`Spaces & Mission Control`、button 5=`Zoom`とする。
- `none`のbutton入力はgesture入力として抑制・変換せず、通常mouse入力として通過させる。
- `Scroll & Navigate`は押下中のmouse moveとwheelを2次元`scroll`へ変換する。名称に`Navigate`を含むが、`NavigationSwipe` adapterの生成能力があることだけでページ戻る/進むまで動作保証済みとは扱わない。ページnavigationの物理contract検証が完了するまでは、その能力を過大に主張しない。
- `Spaces & Mission Control`はボタン押下中のmouse moveを開始時の優勢軸を持つ3本指`DockSwipe`の単一sessionへ変換する。途中の符号反転は同じ軸・sessionで連続投稿する。
- `Zoom`はmagnification系列へ接続する。方向、係数、物理contractが未確定の範囲は試用能力として扱い、完成を主張しない。
- session途中で方向が変わっても別actionへ切り替えず、同じsession IDとlifecycleを維持する。
- 上下左右の方向別bindingとfamilyを方向で選ぶ設定を製品設定と設定UIに持たない。mode選択は方向ではなくbutton 3 / 4 / 5単位で行う。
- 方向ロックと軸ずれ比によるcancelを製品入力経路に持たない。最大継続時間、無入力時間、kill switchなど方向に依存しない安全停止は維持する。
- `NavigationSwipe` adapterの生成能力は維持するが、`Scroll & Navigate`の名称や`scroll`出力を、そのままページnavigationの完成証跡とは扱わない。
- ボタン解放時はactive sessionを`ended`または`cancelled`で必ず閉じ、必要な場合だけmomentumへ移行して通常mouse状態へ戻る。
- 既存の方向別bindingを含む設定は、安全な移行処理で廃止項目を取り除く。旧bindingを暗黙の出力選択へ流用しない。

## 影響

- 設定UIと設定schemaから方向別action選択がなくなり、button 3 / 4 / 5ごとのmode選択と、感度、加速度、慣性、方向非依存のキャンセル条件、対象device条件などを扱う。
- 方向反転や斜め移動を含む入力は、別actionの再認識ではなく開始時に固定した同一軸・session内の連続サンプルとして検証する。
- `GestureAction`全ケースを設定UIに露出することを決めた[ADR-0012](0012-settings-ui-gesture-action-coverage.md)を置き換える。
- trackpad event contract、system-wide投稿、未知contractでのfail closedは[ADR-0036](0036-emulate-trackpad-driver-output-events.md)以降の決定を維持する。

## 関連

- [Issue #144](https://github.com/char5742/nape-gesture/issues/144)
- [ゴール要件](../requirements.md)
- [trackpad driver上位出力eventを再現する](0036-emulate-trackpad-driver-output-events.md)
- [製品gesture出力と診断event出力を分離する](0037-separate-product-and-diagnostic-event-output.md)
