...
- 状態: 採択（timestamp順序とcapture wall-clockはADR-0040、投稿後raw field 39 / 40の解釈はADR-0043で一部置換）
- 日付: 2026-07-11
...
- `captureIndex`は0始まりで欠落なく増加し、全recordのmetadataは完全一致しなければならない。当初のtimestamp非減少要件は、純正trackpad実測に基づき[ADR-0040](0040-capture-order-and-event-timestamp.md)で置き換えた。
...
- capture log上の製品eventは生成markerを必須にする。scroll familyはtype 22を`scroll`、type 29 companion / envelopeを`gesture`として区別し、declared event kindをactual typeと照合する。gestureは既知のscroll / key / pointer / button / null以外でなければならない。provenance上の対象PID、Accessibility、keyboard shortcut、key / pointer / button配送を失敗にする。
- 採択当初は、capture logのraw target process fieldが非0なら明示的な対象PID投稿と判定して失敗にする方針だった。その後、system-wide投稿後にWindowServerが前面の実配送先をraw field 39 / 40へ付与することを実測と回帰テストで確認したため、この判定を[ADR-0043](0043-trackpad-scroll-product-output.md)で更新した。
- 現在は、adapterがpost operation直前のraw field 39 / 40=`0`を検証し、成功投稿直後のdirect post traceへ`delivery: systemWide`を記録する。capture後のfield 39 / 40が非0でも、それ単独では投稿APIの宛先指定を判定せず、失敗にしない。direct post trace、capture log / manifestとのprovenance照合、製品sourceのmodule境界とsource guardを組み合わせて、AX / PID / shortcut経路がないことを固定する。
...
純正trackpad固有のscroll companion、phase / momentum、DockSwipe、NavigationSwipe、magnificationの必須field、順序、OS build別許容差は、Issue #125の物理capture後にPhase 2として固定する。scroll / momentum部分は[ADR-0042](0042-versioned-scroll-momentum-contract-comparison.md)で実装し、残るcandidate familyは追加物理captureまで未確定を維持する。
