...
現行比較schemaは[ADR-0049](0049-fixed-button-to-gesture-class-input.md)に従い、source button、固定GestureClass、X/Y入力量、変換後batch、sample対応、session、terminalを追跡する。`event family`はclass固有adapterの内部contract分類であり、ユーザーmodeまたは製品完成単位として扱わない。
