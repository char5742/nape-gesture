...
現行比較schemaは[ADR-0049](0049-fixed-button-to-finger-count-trackpad-input.md)に従い、source button、finger count、X/Y入力量、変換後量、sample対応、session、terminalを追跡する。`event family`は内部contract分類であり、ユーザーmodeまたは製品完成単位として扱わない。
...
- provenanceはlog SHA-256、capture index、timestamp、event type、output session ID、source button、finger count、source sample order、入力X/Y量、変換後X/Y量、contract ID、event kind、deliveryをrecordごとに持つ。compatibility adapterの内部分類が必要な場合だけevent familyを追加できる。
...
Phase 2では純正trackpadの2 / 3 / 4本指を同一schemaで物理収録し、finger count、X/Y量、phase、timestamp、順序、terminal、OS build別許容差を固定する。既存のscroll / momentum fixtureは[ADR-0042](0042-versioned-scroll-momentum-contract-comparison.md)に基づく2本指内部contractの部分資料として再利用できるが、button 3 / 4 / 5の固定変換を単独では証明しない。
