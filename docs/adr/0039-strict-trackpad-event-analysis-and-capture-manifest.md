...
- 状態: 採択（timestamp順序とcapture wall-clockはADR-0040で一部置換）
- 日付: 2026-07-11
...
- capture log上の製品eventは生成markerを必須にする。scroll familyはtype 22を`scroll`、type 29 companion / envelopeを`gesture`として区別し、declared event kindをactual typeと照合する。gestureは既知のscroll / key / pointer / button / null以外でなければならない。raw target process fieldが非0のevent、またはprovenance上の対象PID、Accessibility、keyboard shortcut、key / pointer / button配送を失敗にする。
