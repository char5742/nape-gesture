...
- ログ由来の候補値は `nape-gesture derive-parameters <path> [--json] [--assert-complete]` で出す。
...
- `--assert-complete` は `acceleration` または `momentum` 候補が未導出、または `warnings` がある場合に非ゼロ終了する。
- `scripts/collect-completion-evidence.sh` は `Fixtures/sample-tuning-trackpad-log.jsonl` の再導出結果を `--assert-complete` 付きで保存し、純正トラックパッド実測ログ取得前でも解析パイプラインを回帰確認する。
- `scripts/collect-completion-evidence.sh` は不足ログと合成 timestamp ログの期待失敗も保存し、短いログや警告付きログを完了証跡として扱わないことを確認する。

## 影響

- Issue #8 は、実機ログ取得前でも再導出ロジックと証跡保存手順を先に検証できる。
- 実測ログ取得後は、同じ CLI で候補値、未導出理由、完了判定の終了コードを Issue コメントへ残せる。
