...
button 3 / 4 / 5を`twoFingerScrollSwipe`、`threeFingerSystemSwipe`、`pinch`の固定GestureClassへ接続し、未押下時は通常mouseをそのまま通すことが唯一の製品モデルである。
...
- button 3 / 4 / 5が3つの固定GestureClassへ一意に接続されている
...
- 旧設定migrationが旧modeを別の結果actionへ流用せず、固定button-to-GestureClass modelへ安全に廃止または正規化する
...
- class固有のversioned単位変換contractについて、係数、clamp、許容差が登録fixtureへ追跡でき、ユーザー調整またはapplication別の係数がない
...
- GUIはbutton 3 / 4 / 5と固定GestureClassを説明し、結果別mode selectorを表示しない
