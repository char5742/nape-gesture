...
| 通常クリック / ドラッグ / ホイール通過 | ジェスチャーボタン未押下時と解放後に通常入力が過剰抑制されない target log | `scripts/collect-runtime-event-evidence.sh`、`normal-after-release`、`analyze-target-log --json --assert-has-unmarked-input`、通常入力通過シナリオの system-test | 実行主体へのアクセシビリティ許可。実デバイスで通常クリック、ドラッグ、ホイールを最終確認する場合の物理操作 | `未設定` | #6, #16 | `人間作業待ち` |
...
| キルスイッチ | `Control + Option + Command + G` で生成と慣性が止まり、再有効化条件が限定される証跡 | `RuntimeSafetyState` の回帰テスト、`system-test run --scenario kill-switch --dry-run --log-json`、`scripts/collect-runtime-event-evidence.sh`、daemon 停止ログ、target log、`analyze-target-log --assert-no-leaks` | 実行主体へのアクセシビリティ許可。物理キーボード由来イベントまで最終証跡に含める場合のみ実操作 | `未設定` | #12, #16 | `人間作業待ち` |
...
`system-test run --scenario kill-switch --dry-run --log-json` は、物理キーボード操作なしで `Control + Option + Command + G` 相当の未マーク `keyDown` / `keyUp` を生成できることを記録します。
