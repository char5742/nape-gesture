...
| ページ戻る / 進む / ズーム / 横スクロール | Safari または Reference Target App でのシナリオ別ログ、画面挙動メモ、割り当てとパラメータ | `system-test run --scenario page-back/page-forward/zoom-in/zoom-out/horizontal-scroll --dry-run --log-json`、`analyze-log --assert-system-scenario`、権限付与済み `.app` bundle からの `system-test run --target safari`、Computer Use による Safari 画面状態読み取り、`target --out`、`analyze-target-log --assert-has-generated-event --assert-has-foreground-capture` | CGEvent 合成 scrollWheel 単体では Safari Web content の `scrollX` / `scrollY` が変化しないため、`.horizontal` はポインタ直下が `AXWebArea` の場合に horizontal scrollbar 経路へ切り替える。Reference Target App の `sendEvent` / `localMonitor` 受信と Safari の `scrollX=0 -> 1272 -> 0` は機械確認済み | `machine-evidence-pr59-main-dddb583/system-test-dry-run/system-page-*`, `machine-evidence-pr59-main-dddb583/system-test-dry-run/system-zoom-*`, `machine-evidence-pr59-main-dddb583/system-test-dry-run/system-horizontal-scroll*`, 2026-07-10 Safari 実動作: `page-back auto-page-2 -> auto-page-1`, `page-forward auto-page-1 -> auto-page-2`, `zoom-out inner-width 655 -> 786`, `zoom-in inner-width 786 -> 655`, `horizontal-scroll scrollX 0 -> 1272`, `generate-scroll --x -1600 scrollX 1272 -> 0`, [ADR-0034](adr/0034-browser-discrete-shortcut-posting.md), [ADR-0035](adr/0035-scroll-event-target-process-routing.md) | #10, #16 | `一部完了` |
