...
release buildの`/Applications/Nape Gesture.app`はインストール済みで、doctor runtime readyを確認している。固定button認識から`scroll` / `dockSwipe` / `dockSwipePinch`をsystem-wideへ投稿する経路があり、system-testではDockが3本指の水平`DockSwipe`で左右のSpaceを切り替え、垂直`DockSwipe`とmotion 4の正負両方向を受理済みである。現在のmacOS設定ではApp Exposéがオフのため、その画面結果は未確認である。

完成を妨げている最優先課題は、Nape Pro実機でbutton 4 / 5を操作し、source入力、生成event、OS結果、terminal、解放後passthroughを一続きに受入することである。純正trackpadとの比較と公開配布署名も未完了である。
...
| --- | --- | --- | --- | --- |
| 固定GestureClass | button 3 / 4 / 5が常に2本指scroll / 3本指system swipe / 4本指system pinch classを選び、session途中で変わらない | 全button、追加button、曖昧同時押下、旧設定migration、GUI read-only、doctorのtest | 残るNape Pro button 4 / 5のsource / generated capture | `実機待ち` |
| source sample保存 | 各accepted move / wheel sampleがexact timestampとcapture orderを保つ1 commandになり、drop、duplicate、coalesce、reorderがない | 正負、斜め、停止、反転、異間隔、長時間、move / wheel混在、queue圧迫のtest | Nape Pro sourceとruntime command / generated batchの照合 | `実機待ち` |
| ProductOutput | 2本指はtype 22 scroll + type 29 companion、3本指はtype 30 DockSwipe motion 1 / 2、4本指はtype 30 DockSwipe motion 4をclass固有contractでsystem-wide投稿する | 3 classのfamily mapping、field、phase、単位、batch、system-wide direct post smoke。Dockの3本指垂直とmotion 4両方向は受理済み | 純正trackpad fixture比較とNape Pro button 4 / 5受入 | `実機待ち` |
...
| passthrough | 未押下、対象外button、対象外device、session終了後に通常click、move、drag、wheelを変更・抑制・再生成しない | event種別identity、生成0、抑制0、解放境界、failure後復帰 | Nape Proと通常mouseの前面App target log | `実機待ち` |
| fail closed | unsupported build、scroll contract / model / DockSwipe templateのfixture / hash / schema不一致、device不一致、TCC不足、session不整合、event失敗でruntime全体の新規抑制を開始せず、別経路へfallbackしない | failure injection、readiness、明示path不正、partial post、terminal retry、product boundary guard | 実利用`.app`で誤出力0と通常入力復帰 | `実機待ち` |
...
1. 最終bundleと`doctor.runtimeIdentity`を一致させ、runtime readyを確認する。
2. Nape Proで未完了のbutton 4 / 5を個別に押し、各classのsource / generated / system-wide captureを取得する。
3. 純正trackpadで対応するscroll、system swipe、pinchを同じOS buildで取得し、class固有contractを比較する。
4. 未押下、各button解放直後、kill switch、device切断、sleepまたはTCC喪失後のpassthroughを取得する。
5. OS / App結果と体感差を低レベルcontractとは別scenarioで記録する。
6. 公開配布物で署名、公証、stapler、Gatekeeper、初回起動を確認する。
