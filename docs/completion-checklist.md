...
次の証跡は、機械証跡は証跡取得対象 commit `dddb58306f5971dace4527d0457b201827dc4554` の main、GUI アプリ化後の runtime event blocker は main `74d5abe` に対して採用します。
...
- GUI アプリ runtime event blocker root: `artifacts/completion/2026-07-09/runtime-event-gui-app-main-74d5abe`
- GUI アプリ runtime event blocker summary: `artifacts/completion/2026-07-09/runtime-event-gui-app-main-74d5abe/summary.md`
- GUI アプリ runtime event blocker status: `artifacts/completion/2026-07-09/runtime-event-gui-app-main-74d5abe/status.json`
- GUI アプリ runtime event blocker doctor: `artifacts/completion/2026-07-09/runtime-event-gui-app-main-74d5abe/doctor/doctor-debug.json`
- GUI アプリ runtime event blocker Issue コメント: [#16](https://github.com/char5742/nape-gesture/issues/16#issuecomment-4920025612), [#6](https://github.com/char5742/nape-gesture/issues/6#issuecomment-4920025616), [#12](https://github.com/char5742/nape-gesture/issues/12#issuecomment-4920025613)
...
`runtime-event-gui-app-main-74d5abe/status.json` では `status: "blocked"`、`blockerCode: "accessibility.missing"` として機械判定できます。
...
| doctor 権限・runtimeIdentity | 実利用する `.app` または実行ファイルでの `doctor --probe-hid --benchmark-events ... --json --assert-runtime-ready` | `doctor --json` の `runtimeIdentity`、`runtimeReadiness`、`tccStatus`、`tccStatus.permissionTarget`、`grantRequired`、`targetDeviceDiagnostics`、`settingsValidationIssues`、benchmark 部分、`--assert-runtime-ready` の期待失敗 code | システム設定でアクセシビリティと入力監視を実利用主体へ許可し、権限反映後に再実行する操作 | `machine-evidence-pr59-main-dddb583/doctor-and-performance/`, `runtime-event-gui-app-main-74d5abe/doctor/doctor-debug.json`, [GUI アプリ blocker コメント](https://github.com/char5742/nape-gesture/issues/16#issuecomment-4920025612) | #11, #13, #15, #16 | `人間作業待ち` |
