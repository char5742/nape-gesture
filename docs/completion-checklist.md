...
`sample-cpu --json --assert-baseline` は、指定 PID の CPU 使用率を `processCpuSampling` として保存し、idle / active / recovery の平均基準を終了コードで判定します。completion evidence の短時間 smoke はコマンド形式の退行確認であり、日常利用主体の常駐 CPU 完了証跡ではありません。
