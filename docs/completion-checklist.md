...
| 設定 UI / 固定trackpad gesture | `.app` 起動時に設定ウィンドウが開き、方向別割り当てを表示せず、activation button、感度、加速度、慣性、方向非依存のキャンセル条件、対象デバイス、対象入力の紐づけ秒を編集できる証跡。mouse move→`DockSwipe`、wheel→`scroll`の固定操作と、旧binding設定の安全なmigrationを含む | `SettingsUIField.descriptors`が方向別bindingを含まないcore test、開始軸とsessionを維持して符号反転するproduct test、旧bindingを含む実設定ファイルの原子的migration、`GUIAppLaunchPresenter.regularGUIApp`、設定バリデーション、bundle identity、`gui-smoke --json --assert`、computer-useによる方向別割り当て非表示 | なし。TCC許可、実イベント投稿、Nape Pro実機操作は別行で扱う | Issue #144のcore / product / migration test、release bundle検証、GUI smoke、computer-use AX treeで方向別項目なしを確認。PR artifactへの永続化は統合時に行う | #11, #16, #144 | `進行中` |
...
`SettingsUIField.descriptors` の core test は、設定 UI が activation button、対象入力の紐づけ秒、感度、加速度、慣性、方向非依存のキャンセル条件、対象デバイスの設定パスを網羅し、方向別bindingを含まないこと、control kind、表示名重複なし、設定パス重複なし、アプリ別設定なし、JSON round-trip を満たすことを記録します。
...
固定連続2D操作の core / product test は、ボタン押下中のX / Y入力が途中で方向転換しても別actionへ切り替わらず、押下開始から解放または明示cancelまで同じsession IDとlifecycleを維持することを記録します。旧binding migration testは、方向別bindingを含む既存設定を安全に読み込み、廃止項目を取り除き、旧値を暗黙の出力選択へ流用せず、保存後の設定と製品surfaceに方向別bindingが残らないことを記録します。
