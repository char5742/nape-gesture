...
証跡収集スクリプトは実行ビットなしで管理し、必ず `sh` 経由で実行します。`scripts/fixtures/sample-cpu-self-exec.sh` も実行ビットを付けず、回帰スクリプトから `/bin/bash` で読み込みます。
...
スクリプトは `commands.txt` と `summary.md` を出力し、由来ガード、debug / release build、core tests、app bundle 作成と検証、bundle identity exact check、`LSUIElement=false` の通常 GUI app check、artifact 配下 config を使った `gui-smoke --json --assert` による AppKit 内 GUI smoke、未署名時の署名必須検証の期待失敗、ad-hoc 署名、署名済み bundle 検証、ライセンス原本一致、doctor、`doctor --json` の `runtimeReadiness` / `tccStatus` / `permissionTarget` / `grantRequired` / `targetDeviceDiagnostics` 存在確認、`doctor --probe-hid`、`doctor --assert-runtime-ready` の期待失敗と failure code、`benchmark --assert-baseline`、benchmark JSON の `schemaVersion: 3` / batch p95 / p99 field check、`sample-cpu` の duration coverage、初回有効 sample 後の ready handshake、ready の no-replace 16並列競合、out-ready の symlink・`..`・非存在 component・相対 path・case-only 衝突、PID / 浮動小数点 / sample 数境界、path 変更 exec、同一 path exec、`processIDVersion` 回帰と直接起動した `/bin/sleep` smoke、system-test dry-run と `analyze-log --assert-system-scenario`、生成スクロール dry-run、fixture 解析、`derive-parameters --json --assert-complete`、`devices --all --json` を収集します。
...
`sample-cpu --expected-executable <path> --json --assert-baseline` は、指定 PID の実行ファイル、開始時刻トークン、audit token の `(pid, pidversion)` を各 sample の前後で固定し、CPU 使用率を `processCpuSampling` として保存します。単調 uptime の deadline 到達後まで採取し、`requestedDurationReached` が false の場合や `expectedExecutablePath`、`resolvedExecutablePath`、`executableIdentityMatched`、`processStartToken`、`processIDVersion`、`processIdentityStable` の不一致・取得不能は CPU 平均が基準内でも不合格です。`--ready-file` は1件目の有効 sample と前後 identity 照合後に `completedSampleCount: 1` を含む完全 JSON を一時 sibling から `RENAME_EXCL` で排他的に公開し、exec 回帰の trigger はその作成を待ちます。16並列競合では成功を1件だけとし、既存 ready の上書きと一時ファイル残留を許しません。completion evidence の短時間 smoke は、直接起動した `/bin/sleep` と `$!` の組み合わせによるコマンド形式の退行確認であり、日常利用主体の常駐 CPU 完了証跡ではありません。
