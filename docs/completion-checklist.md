...
| Trackpad output event logger | 純正trackpadのevent type / subtype / raw field / serialized dataをJSON Linesへ保存できる | `trackpad-event-log --duration --out`、Codable round-trip、capture順、zeroを含むordered field scan、非有限値bit pattern、serialized event再構築、bounded queue、SIGINT drain、0件期待失敗、合成event smoke | なし | [2026-07-11 local smoke](evidence/2026-07-11-trackpad-event-logger-local-smoke.md)。commit後CIと専用analyzer証跡は未設定 | #117, #118, #129, #132 | `一部完了` |
