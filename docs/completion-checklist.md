...
`system-test readiness --json --assert` は、Issue #9 / #10 に紐づく scenario、前段の機械証跡コマンド、画面挙動実測待ち、`need:human` 境界を同じ JSON schema で記録します。`system-test matrix` は同じ出力の別名で、completion evidence では alias の退行も確認します。この成功は画面挙動の完了ではなく、実測へ進む前の readiness 証跡です。
