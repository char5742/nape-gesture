...
`generate-scroll --dry-run --log-json` と全期待値付き `analyze-log --json --assert-generated-scroll-log` は、純正ログと比較する前の space-right 投稿前 InputLog が通常30件、momentum changed 8件、ended-zero 1件、正X、通常X合計1200、per-step point/scroll量子化、`isContinuous == 1`、timestamp厳密増加、通常 `began, changed*, ended` と momentum `changed+, ended-zero` の exact 列を満たすことを記録します。1 step、momentumなし、サブ1 point、`momentum-decay=0` もgeneratorの有効な境界として成功させます。
`Fixtures/generated-scroll-auto-valid.jsonl` は39件成功を固定し、その他の `generated-scroll-auto-*.jsonl` は切り詰め、途中欠落、正負混在、合計相殺、逆方向、量不足、point/scroll不一致、ended欠落、順序逆転、未知/欠落/混在phase、同一timestamp、重複、changed-zero終端、終了後tail、`isContinuous=2`、空ログを expected failure として固定します。ここで changed-zero終端は ended phase 欠落を指し、途中のゼロ momentum changed 自体は許可します。`Fixtures/sample-generated-scroll-log.jsonl` は比較専用 sample として assertion の expected failure にします。明示 phase override の期待 mode は未対応として非ゼロ終了します。
