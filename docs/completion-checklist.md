...
`generate-scroll --dry-run --log-json` と全期待値付き `analyze-log --json --assert-generated-scroll-log` は、純正ログと比較する前の space-right 投稿前 InputLog が通常30件、momentum changed 8件、ended-zero 1件、正X、通常X合計1200、`isContinuous == 1`、timestamp厳密増加、通常 `began, changed*, ended` と momentum `changed+, ended-zero` の exact 列を満たすことを記録します。全非ゼロ record の point/scroll 符号と量子化量も照合します。
