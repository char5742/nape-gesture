...
| ページ戻る / 進む / ズーム / 縦横スクロール | Safari または Reference Target App でのシナリオ別ログ、画面挙動メモ、割り当てとパラメータ | `system-test run --scenario page-back/page-forward/zoom-in/zoom-out/horizontal-scroll --dry-run --log-json`、`analyze-log --assert-system-scenario`、権限付与済み `.app` bundle からの `system-test run --target safari`、`generate-scroll --mode horizontal/free` の縦横 32 step、Computer Use による Safari 画面状態読み取り、`target --out`、`analyze-target-log --assert-has-generated-event --assert-has-foreground-capture` | CGEvent 合成 scrollWheel 単体では Safari Web content の `scrollX` / `scrollY` が変化しないため、実カーソル下が `AXWebArea` の場合は AX scrollbar 経路へ切り替える。PID override なしの通常 async 対象選択と、自動化ホスト下の `--post-to-pid <Safari PID>` 32 step 再検証を分けて機械確認済み | `machine-evidence-pr59-main-dddb583/system-test-dry-run/system-page-*`, `machine-evidence-pr59-main-dddb583/system-test-dry-run/system-zoom-*`, `machine-evidence-pr59-main-dddb583/system-test-dry-run/system-horizontal-scroll*`, 2026-07-10 Safari 実動作: `page-back auto-page-2 -> auto-page-1`, `page-forward auto-page-1 -> auto-page-2`, `zoom-out inner-width 655 -> 786`, `zoom-in inner-width 786 -> 655`, 通常 no-override async `scrollX 0 -> 1609 / scrollY 0 -> 1675 / 両軸 0 へ復帰`, PID 固定 32 step `scrollX 0 -> 1438 / scrollY 0 -> 1675 / 両軸 0 へ復帰`, final6 wall time `0.37〜0.38 秒`, [ADR-0035](adr/0035-browser-discrete-shortcut-posting.md), [ADR-0036](adr/0036-scroll-event-target-process-routing.md) | #10, #16 | `完了` |
