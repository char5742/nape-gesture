...
- System Behavior Test PID target root: `artifacts/completion/2026-07-10/system-test-pid-target-main-6f0c21c`
- System Behavior Test PID target summary: `artifacts/completion/2026-07-10/system-test-pid-target-main-6f0c21c/summary.md`
...
`system-test-pid-target-main-6f0c21c/summary.md` では、`space-left`、`space-right`、`horizontal-scroll` の dry-run、`analyze-log --assert-system-scenario`、Reference Target App PID への直接投稿、`analyze-target-log --assert-has-generated-event --assert-has-foreground-capture` が成功しています。
これは生成スクロールイベントが前面 AppKit 経路へ届く無人証跡であり、純正トラックパッド比較、Nape Pro 実機、Spaces / Mission Control の画面遷移、Safari のページ遷移やズームの画面挙動は完了扱いにしません。
...
`system-test run --scenario space-left/space-right/horizontal-scroll --post-to-pid <Reference Target App PID>` は、対応する生成スクロールイベントが前面 Reference Target App の AppKit 経路に届くことを `analyze-target-log --assert-has-generated-event --assert-has-foreground-capture` で記録します。これは sink 診断を兼ねた無人 foreground 受信証跡であり、Finder / Safari 前面時の実画面挙動や純正トラックパッド比較の代替にはしません。
