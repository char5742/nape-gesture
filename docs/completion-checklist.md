...
スクリプトは `commands.txt` と `summary.md` を出力し、debug / release build、core tests、app bundle 作成と検証、doctor、`doctor --probe-hid`、`benchmark --assert-baseline`、system-test dry-run、生成スクロール dry-run、fixture 解析、`derive-parameters --json`、`devices --all --json` を収集します。
