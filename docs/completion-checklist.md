...
`sample-cpu --expected-executable <path> --json --assert-baseline` は、指定 PID の実行ファイルと開始時刻トークンを各 sample で固定し、CPU 使用率を `processCpuSampling` として保存します。`expectedExecutablePath`、`resolvedExecutablePath`、`executableIdentityMatched`、`processStartToken`、`processIdentityStable` の不一致は CPU 平均が基準内でも不合格です。completion evidence の短時間 smoke は、直接起動した `/bin/sleep` と `$!` の組み合わせによるコマンド形式の退行確認であり、日常利用主体の常駐 CPU 完了証跡ではありません。
