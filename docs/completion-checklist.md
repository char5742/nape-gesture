...
| スリープ復帰 / 抜き差し / 権限変更後復旧 | スリープ復帰、Nape Pro 抜き差し、TCC 権限変更後の停止、再試行、復旧ログ | `RuntimeRecoveryState` の回帰テスト、`doctor --json`、設定検証、権限未許可時のエラー出力 | Mac のスリープ復帰、実デバイス抜き差し、システム設定での TCC 権限変更 | `未設定` | #13, #16 | `人間作業待ち` |
...
スクリプトは `commands.txt` と `summary.md` を出力し、debug / release build、core tests、app bundle 作成と検証、doctor、benchmark、system-test dry-run、生成スクロール dry-run、fixture 解析、`derive-parameters --json`、`devices --all --json` を収集します。
