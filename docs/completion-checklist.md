...
| キルスイッチ | `Control + Option + Command + G` で生成と慣性が止まり、再有効化条件が限定される証跡 | `RuntimeSafetyState` の回帰テスト、`scripts/collect-runtime-event-evidence.sh`、daemon 停止ログ、target log、`analyze-target-log --assert-no-leaks` | 実行主体へのアクセシビリティ許可。物理キーボード由来イベントまで最終証跡に含める場合のみ実操作 | `未設定` | #12, #16 | `人間作業待ち` |
