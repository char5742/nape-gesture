...
`generate-scroll --dry-run --log-json` と `analyze-log --json --assert-generated-scroll-log` は、純正ログと比較する前の生成スクロールログが、Nape Gesture 生成 `scrollWheel` だけ、continuous/precise、timestamp 単調、通常 scroll phase と momentum phase 分離、momentum ゼロ delta 終了を満たすことを記録します。
