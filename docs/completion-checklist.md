...
| ページ戻る / 進む / ズーム / 横スクロール | 時刻修正後の Safari または Reference Target App でのシナリオ別 CGEvent log、画面挙動、割り当てとパラメータ | `system-test run --scenario page-back/page-forward/zoom-in/zoom-out/horizontal-scroll --dry-run --log-json`、`analyze-log --assert-current-uptime --assert-system-scenario`、実投稿 CGEvent log、computer-use、`target --out`、`analyze-target-log --assert-has-gesture` | なし。Safari 画面確認は computer-use で行う。純正トラックパッド操作が必要な比較は別行で扱う | 修正前の PR #101 証跡は不採用。ADR-0037 適用後の再取得待ち | #10, #16, #102 | `機械証跡待ち` |
