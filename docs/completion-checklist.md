...
`pr101-ax-delivery-final6` の top-level Safari 結果は nested scroll / wheel semantics の完成証跡にはしません。review 修正後は [Safari scroll 配送比較](safari-scroll-delivery-verification.md) と `safari-scroll-probe/contract.json` を正とします。今回のローカル Safari 再検証では、unlabeled generic の全値 0 維持、微小横成分を含む縦のみ frame の `frame.y 0 -> 367` / outer 0 維持、長い article の `outer.y 0 -> 674`、top-level の `outer.x 0 -> 1358 / outer.y 0 -> 608` を確認しました。Issue #102 は統合済みですが、統合 commit と TCC 許可済み `.app` identity を固定し、contract の5 assertion、通常 async、PID 固定 sync、端到達、Computer Use の通常 wheel、生成 CGEvent log の current uptime、runtime performance completion を最終再取得する作業は未完了です。
...
| ページ戻る / 進む / ズーム / 縦横スクロール | 時刻修正後の Safari または Reference Target App でのシナリオ別 CGEvent log、画面挙動、割り当てとパラメータ | `system-test run --scenario page-back/page-forward/zoom-in/zoom-out/horizontal-scroll --dry-run --log-json`、`analyze-log --assert-current-uptime --assert-system-scenario`、権限付与済み `.app` bundle の `generate-scroll --mode horizontal/free`、AX selector / clipping / fallback core test、`python3 scripts/check-safari-scroll-probe-contract.py`、実投稿 CGEvent log、Computer Use、`target --out`、`analyze-target-log --assert-has-gesture` | なし。Safari 画面確認は Computer Use で行う。AX が generic overflow 境界を完全に省略する場合の通常 wheel同等 target / `preventDefault()` semantics は未成立として記録する | 修正前の Safari 値は比較専用で完成証跡にしない。ADR-0037 統合 commit と TCC 許可済み `.app` identity で contract の5 assertion、通常 async、PID固定 sync、端到達、Computer Use wheel、runtime performance、current-uptime CGEvent logを再取得待ち。[Safari scroll 配送比較](safari-scroll-delivery-verification.md)、[ADR-0035](adr/0035-browser-discrete-shortcut-posting.md)、[ADR-0036](adr/0036-scroll-event-target-process-routing.md)、[ADR-0037](adr/0037-monotonic-event-time-domain.md) | #10, #16, #102 | `機械証跡待ち` |
