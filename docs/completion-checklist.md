...
`pr101-ax-delivery-final6` の旧top-level結果は比較専用です。完成証跡は `artifacts/completion/2026-07-11/pr101-final-safari` を正とし、単一候補commitとTCC許可済み`.app` identityでcontractの6 assertion / 12 run、通常async、PID固定sync / async、frame端到達、Computer Use wheelをevaluator exit `0` / 全件passで確認します。同rootの`discrete/`ではページ戻る / 進むのURL遷移、ズーム前後のfixture幅、32 step横スクロールの画面変化とcurrent-uptime計画を保存します。`pr101-final-runtime-event`では5 runtimeシナリオとperformance completionを再取得します。Issue #105はCodex hostの一時退避をforeground処理で自動化してclose済みです。
...
| ページ戻る / 進む / ズーム / 縦横スクロール | 時刻修正後の Safari でのシナリオ別計画ログ、画面挙動、割り当て、runtime配送結果 | `system-test run --scenario page-back/page-forward/zoom-in/zoom-out/horizontal-scroll --dry-run --log-json`、`analyze-log --assert-current-uptime --assert-system-scenario`、権限付与済み`.app`の実投稿、AX selector / clipping / fallback core test、Safari static / WebKit render / runtime evaluator、Computer Use | 追加の人間作業なし。Codex host退避はforeground helperで自動化する。AXがgeneric overflow境界を完全に省略する場合の通常wheel同等target / `preventDefault()` semanticsは成立範囲外として維持する | `artifacts/completion/2026-07-11/pr101-final-safari`の6 assertion / 12 run全件pass、通常async window owner、ページURL遷移、ズームfixture幅、32 step横スクロール画面差分、`pr101-final-runtime-event`のruntime performance completion。[Safari scroll 配送比較](safari-scroll-delivery-verification.md)、[ADR-0035](adr/0035-browser-discrete-shortcut-posting.md)、[ADR-0036](adr/0036-scroll-event-target-process-routing.md)、[ADR-0037](adr/0037-monotonic-event-time-domain.md)、[ADR-0038](adr/0038-safari-evidence-pointer-and-routing.md) | #10, #16, #102, #104, #105, #106 | `機械受入完了` |
