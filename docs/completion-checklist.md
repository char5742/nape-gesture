...
`machine-evidence-main-e22aa52/summary.md` では、由来ガード、debug / release build、core tests、`.app` 作成と検証、bundle identity、未署名 bundle の署名必須検証 expected failure、ad-hoc 署名、codesign verify、署名済み bundle 検証、license / third-party notices 同梱一致、AppKit `gui-smoke --json --assert`、`doctor` / `doctor --probe-hid` JSON、runtime readiness / TCC / permission target / targetDeviceDiagnostics field check、`doctor --assert-runtime-ready` の expected failure、benchmark p95 / p99 field check、`system-test readiness --json --assert`、`system-test` dry-run、fixtures 解析、`devices --all --json` が成功している。
...
| Spaces / Mission Control | Finder など前面時の `system-test` 実行ログ、生成イベントログ、画面挙動メモ、公開 API の限界がある場合の根拠 | `system-test readiness --json --assert`、`system-test matrix --json --assert`、`system-test list`、`system-test run --dry-run --log-json`、`space-left/space-right --post-to-pid <Reference Target App PID>` の AppKit foreground 受信ログ、ready diagnostics、`analyze-target-log --assert-has-generated-foreground-capture`、方向 / 件数 / 量 assertion。`mission-control` は key shortcut dry-run のみ | CGEvent 実投稿、Reference Target App、computer-use で代替できない場合だけ、Spaces / Mission Control の画面遷移を実測する操作 | `machine-evidence-pr59-main-dddb583/system-test-dry-run/system-space-*`, `machine-evidence-pr59-main-dddb583/system-test-dry-run/system-mission-control*`, `system-test-pid-target-main-746df2c/summary.md`, [System Behavior Test matrix](system-behavior-matrix.md), [#9 最新コメント](https://github.com/char5742/nape-gesture/issues/9#issuecomment-4927198152), [#16 最新コメント](https://github.com/char5742/nape-gesture/issues/16#issuecomment-4927198158) | #9, #16 | `一部完了` |
| ページ戻る / 進む / ズーム / 横スクロール | Safari または Reference Target App でのシナリオ別ログ、画面挙動メモ、割り当てとパラメータ | `system-test readiness --json --assert`、`system-test run --scenario page-back/page-forward/zoom-in/zoom-out/horizontal-scroll --dry-run --log-json`、`analyze-log`、`horizontal-scroll --post-to-pid <Reference Target App PID>` の AppKit foreground 受信ログ、`target --out`、`analyze-target-log --assert-has-gesture`。ページ戻る / 進む / ズームは key shortcut dry-run のみ | CGEvent 実投稿、Reference Target App、computer-use で代替できない場合だけ、Safari など対象アプリでページ遷移、ズーム、横スクロールを実操作して確認 | `machine-evidence-pr59-main-dddb583/system-test-dry-run/system-page-*`, `machine-evidence-pr59-main-dddb583/system-test-dry-run/system-zoom-*`, `machine-evidence-pr59-main-dddb583/system-test-dry-run/system-horizontal-scroll*`, `system-test-pid-target-main-746df2c/summary.md`, [System Behavior Test matrix](system-behavior-matrix.md), [#10 最新コメント](https://github.com/char5742/nape-gesture/issues/10#issuecomment-4927198165), [#16 最新コメント](https://github.com/char5742/nape-gesture/issues/16#issuecomment-4927198158) | #10, #16 | `一部完了` |
...
`system-test readiness --json --assert` は、全 scenario の exact Issue 集合、expected target、event shape、前段機械証跡、runtime / target 証跡対象8件、画面挙動待ち8件、`need:human` 境界、completion state を同じ JSON schema で記録します。Markdown の runtime / target 列、値なし `--out`、競合 format、未知 option の expected failure も completion evidence で固定します。`system-test matrix` は同じ出力の別名です。この成功は画面挙動の完了ではなく、実測へ進む前の readiness 証跡です。
