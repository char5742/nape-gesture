...
`sh scripts/check-provenance.sh` はtracked filesだけを対象とし、製品surfaceから除外する外部固有名の表記揺れをallowlistなしで検出し、README / PR template / PR review checklistのrepo-local由来方針を確認します。`THIRD_PARTY_NOTICES.md`は実際に同梱する依存通知に限定します。このチェックは法的な完全証明ではなく、採用根拠と識別子境界の早期検出ガードとして扱います。
