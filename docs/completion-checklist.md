...
| Trackpad raw analyzer Phase 1 | 現行JSON Lines、manifest、serialized event、generated配送を機械判定できる | 厳格parser negative tests、nesting上限、nullable subtype、unknown field保持、manifest原子書き込みと改ざん検出、CGEvent再構築、生成marker / actual type / raw target process field、provenanceのSHA / 件数 / 順序 / timestamp / type一致、製品source境界guard、CLI正常系と非ゼロ終了 | なし | core tests、synthetic serialized eventのlocal CLI smoke、8本の純正captureに対するstructure / manifest / host pass。fixture contract比較はPhase 2で実装する | #117, #125, #129 | `Phase 1完了` |
...
| 純正トラックパッド比較 | 純正trackpadと生成eventを同一schemaで保存したtype、subtype、field、phase、momentum、timestamp、順序の比較結果 | logger / analyzer、[25F80小型fixture](../Fixtures/trackpad-contract/25F80/physical-observations.json)、差分report | ready同期後にNavigationSwipe左右、pinch方向marker、DockSwipe反対方向 / cancel、Mission Control / App Exposéを追加収録する | [2026-07-11物理capture](evidence/2026-07-11-physical-trackpad-contract-capture.md)。scroll / momentumは採用可能、他familyはcandidate /未取得を明示 | #7, #8, #117, #118, #125, #129 | `一部完了` |
