...
`pr101-ax-delivery-final6` の top-level Safari 結果は nested scroll / wheel semantics の完成証跡にはしません。review 修正後は [Safari scroll 配送比較](safari-scroll-delivery-verification.md) と `safari-scroll-probe/contract.json` を正とします。今回のローカル Safari 再検証では、unlabeled generic の全値 0 維持、微小横成分を含む縦のみ frame の `frame.y 0 -> 367` / outer 0 維持、長い article の `outer.y 0 -> 674`、top-level の `outer.x 0 -> 1358 / outer.y 0 -> 608` を確認しました。Issue #102 取り込み後に同じ5 assertionと runtime performance を同一 head で最終再取得する作業は未完了です。
...
| ページ戻る / 進む / ズーム / 縦横スクロール | Safari または Reference Target App でのシナリオ別ログ、画面挙動メモ、割り当てとパラメータ | `system-test run --scenario page-back/page-forward/zoom-in/zoom-out/horizontal-scroll --dry-run --log-json`、`analyze-log --assert-system-scenario`、権限付与済み `.app` bundle の `generate-scroll --mode horizontal/free`、AX selector / clipping / fallback core test、`python3 scripts/check-safari-scroll-probe-contract.py`、repo Safari fixture、Computer Use、`scripts/probe-cgevent-scroll-delivery.swift`、CLI expected failure | 今回 fixture の unlabeled generic fail-closed、微小横成分を含む縦のみ frame、長い article、top-level 縦横はローカル Safari で確認済み。Issue #102 取り込み後に同じ5 assertionと runtime performance を最終再取得する。AX が generic overflow 境界を完全に省略する場合の通常 wheel同等 target / `preventDefault()` semantics は未成立 | `machine-evidence-pr59-main-dddb583/system-test-dry-run/system-page-*`, `machine-evidence-pr59-main-dddb583/system-test-dry-run/system-zoom-*`, `machine-evidence-pr59-main-dddb583/system-test-dry-run/system-horizontal-scroll*`, 今回 Safari generic 全値 0、frame `y 0 -> 367` / outer 0、article outer `0 -> 674`、top-level `x 0 -> 1358 / y 0 -> 608`、Computer Use generic wheel `inner=682 / wheel=1`, [Safari scroll 配送比較](safari-scroll-delivery-verification.md), [ADR-0035](adr/0035-browser-discrete-shortcut-posting.md), [ADR-0036](adr/0036-scroll-event-target-process-routing.md) | #10, #16 | `一部完了` |
...
`system-test run --scenario page-back/page-forward/zoom-in/zoom-out --dry-run --log-json` と `analyze-log --json --assert-system-scenario <name>` は、Safari や対応アプリで実操作する前に、`systemTestScenario`、`sequenceIndex`、離散割り当ての修飾キー down/up を含む key event 列、keyCode、exact modifier flags を同じ JSON Lines 形式と終了コードで記録します。
