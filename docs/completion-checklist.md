...
| 元入力抑制 | ジェスチャー成立後に未マークのクリック、ドラッグ、ホイール、キーが前面アプリへ漏れない target log | `scripts/collect-runtime-event-evidence.sh`、`target --out`、`system-test run`、`analyze-target-log --assert-no-leaks`、fixtures 回帰 | 実行主体へのアクセシビリティ許可。Nape Pro 実機で最終確認する場合は成立ジェスチャー操作 | `未設定` | #6, #16 | `人間作業待ち` |
