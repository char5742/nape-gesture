...
| 純正トラックパッド比較 | 純正トラックパッド操作ログ、生成イベントログ、比較結果、差分理由 | `generate-scroll --dry-run --log-json`、`derive-parameters --json`、`compare-log`、保存済みログ解析 | 純正トラックパッドで Spaces、スクロール、ズームなどを操作してログを取る作業 | `未設定` | #7, #8, #9, #10, #16 | `人間作業待ち` |
...
スクリプトは `commands.txt` と `summary.md` を出力し、debug / release build、core tests、app bundle 作成と検証、未署名時の署名必須検証の期待失敗、ad-hoc 署名、署名済み bundle 検証、ライセンス原本一致、doctor、`doctor --probe-hid`、`benchmark --assert-baseline`、system-test dry-run、生成スクロール dry-run、fixture 解析、`derive-parameters --json`、`devices --all --json` を収集します。
...
`Fixtures/sample-tuning-trackpad-log.jsonl` の `derive-parameters --json` は、純正トラックパッド実測ログ取得後に deadZone、加速度、慣性候補と未導出理由を同じ形式で保存できることを記録します。
