...
Safari runtime 証跡はroot直下の `safari-scroll-runtime-manifest.json` と evaluator stdoutの `evaluation.json` を保存します。manifestは候補commit、`.app` executable、正本contract、全参照artifactのSHA-256を固定し、evaluatorへ同じcommitと実行ファイルpathを明示します。completion collector経由では評価JSONをcollector rootの`safari-scroll-runtime/`にも保存します。評価結果はexit `0`、`status=pass`、`failureCount=0`、`blockedCount=0`である場合だけ採用します。
...
これらは Issue #102 の時刻修正前に取得されている。元入力抑制、GUI、権限など時刻不整合に依存しない判定は維持するが、慣性、生成 CGEvent timestamp、Safari 画面挙動、runtime performance の command timestamp は修正後証跡へ置き換えるまで完了根拠にしない。
特に PR #101 の Safari 診断は epoch timestamp 投稿が混在した可能性を除外できないため、Issue #10 / #16 のページ戻る / 進む、ズーム、縦横スクロール証跡として過大評価しない。
...
- Issue #102 単調時刻機械証跡 root: `artifacts/completion/2026-07-10/issue102-monotonic-event-time`
- Issue #102 単調時刻機械証跡 summary: `artifacts/completion/2026-07-10/issue102-monotonic-event-time/summary.md`
- Issue #102 単調時刻機械証跡: Date epoch 混入ガード、debug / release build、core tests、全 system-test current uptime assertion、generate-scroll current uptime assertion、派生値overflow・後続timestamp変換不能・部分出力禁止、epoch fixture expected failure が成功
...
NAPE_COMPLETION_ARTIFACT_ROOT=artifacts/completion/2026-07-10/issue102-monotonic-event-time sh scripts/collect-completion-evidence.sh
...
`issue102-monotonic-event-time-final-review-2/summary.md` では、上記に加えて CGEvent 関連経路の Date epoch 混入ガード、全 system-test dry-run と generate-scroll dry-run の current uptime assertion、派生値overflow・後続timestamp変換不能のexpected failureとstdout空判定、epoch timestamp fixture の expected failure が成功している。これは Safari 画面挙動や実イベント投稿の再取得を代替しない。
...
`artifacts/completion/2026-07-10/pr101-ax-delivery-final6` でも同じ5シナリオが全件成功し、runtime performance schema 2 の provisional / completion 解決後の `deferredDeliveryRecordCount` は全て 0、Spaces / gesture-wheel / gesture-wheel-then-kill-switch の tap callback から実配送完了までの p95 は 0.10〜1.76ms で基準内です。AppKit 受信と画面反映はこの値に含みません。
`pr101-ax-delivery-final6` の top-level Safari 結果は nested scroll / wheel semantics の完成証跡にはしません。review 修正後は [Safari scroll 配送比較](safari-scroll-delivery-verification.md) と `safari-scroll-probe/contract.json` を正とします。今回のローカル Safari 再検証では、unlabeled generic の全値 0 維持、微小横成分を含む縦のみ frame の `frame.y 0 -> 367` / outer 0 維持、長い article の `outer.y 0 -> 674`、top-level の `outer.x 0 -> 1358 / outer.y 0 -> 608` を確認しました。Issue #102 は統合済みですが、単一候補commitとTCC許可済み`.app` identityを固定し、contractの6 assertion / 12 run、通常async、PID固定sync / async、frame端到達、Computer Useの通常wheel、生成CGEvent logのcurrent uptime、runtime performance completionを最終再取得する作業は未完了です。Issue #106のruntime evaluatorは合成artifact回帰まで実装済みで、実artifactのpassとIssue #105の通常routing取得は未完了です。
...
| 起動後単調イベント時刻 | 実入力、HID、慣性、生成 CGEvent、runtime 性能が同じ uptime ドメインであり、epoch 混入を拒否する証跡 | `MonotonicEventClock` と慣性の core test、`generate-scroll` / `system-test` dry-run、`analyze-log --assert-current-uptime`、派生値overflow・後続timestamp変換不能・部分出力禁止、epoch fixture expected failure、Date epoch 混入ガード | なし。Safari 画面挙動は computer-use、純正トラックパッドと Nape Pro は別行で扱う | `artifacts/completion/2026-07-10/issue102-monotonic-event-time-final-review-2/summary.md` は統合前 branch の参考証跡。PR #101 と Issue #102 の統合 commit SHA で completion を再収集する | #7, #10, #14, #16, #102 | `一部完了` |
...
| doctor 権限・runtimeIdentity | 実利用する `.app` または実行ファイルでの `doctor --probe-hid --benchmark-events ... --json --assert-runtime-ready` | `doctor --json` の `runtimeIdentity`、`runtimeReadiness`、`tccStatus`、`tccStatus.permissionTarget`、`grantRequired`、`targetDeviceDiagnostics`、`settingsValidationIssues`、benchmark 部分、`--assert-runtime-ready` の期待失敗 code、`PermissionRecoveryPresenter` の権限別 System Settings 導線 core test | なし。2026-07-10 時点で `.build/NapeGesture.app` のアクセシビリティと入力監視は許可済み | `machine-evidence-pr59-main-dddb583/doctor-and-performance/`, `runtime-event-kill-switch-release-suppression/doctor/doctor-debug.json`, `artifacts/completion/2026-07-10/pr101-ax-delivery-final6/doctor/doctor-debug.json`, `gui-computer-use-main-f43e217/doctor/doctor-app.json`, [runtime event 最新コメント](https://github.com/char5742/nape-gesture/issues/16#issuecomment-4926725190) | #11, #13, #15, #16, #74 | `完了` |
...
| ページ戻る / 進む / ズーム / 縦横スクロール | 時刻修正後の Safari または Reference Target App でのシナリオ別 CGEvent log、画面挙動、割り当てとパラメータ | `system-test run --scenario page-back/page-forward/zoom-in/zoom-out/horizontal-scroll --dry-run --log-json`、`analyze-log --assert-current-uptime --assert-system-scenario`、権限付与済み `.app` bundle の `generate-scroll --mode horizontal/free`、AX selector / clipping / fallback core test、Safari contract static checker、WebKit render checker、runtime evaluator正負・blockedテスト、実artifact evaluator、実投稿CGEvent log、Computer Use、`target --out`、`analyze-target-log --assert-has-gesture` | 通常async取得直前にself-host windowを一時退避する操作だけIssue #105で人間作業とする。Safari画面確認はそれ以外Computer Useで行う。AXがgeneric overflow境界を完全に省略する場合の通常wheel同等target / `preventDefault()` semanticsは未成立として記録する | 修正前のSafari値は比較専用で完成証跡にしない。単一候補commitとTCC許可済み`.app` identityでcontractの6 assertion / 12 run、通常async、PID固定sync / async、frame端到達、Computer Use wheel、runtime performance、current-uptime CGEvent logを再取得し、evaluator exit `0` / passを必要条件とする。[Safari scroll 配送比較](safari-scroll-delivery-verification.md)、[ADR-0035](adr/0035-browser-discrete-shortcut-posting.md)、[ADR-0036](adr/0036-scroll-event-target-process-routing.md)、[ADR-0037](adr/0037-monotonic-event-time-domain.md)、[ADR-0038](adr/0038-safari-evidence-pointer-and-routing.md) | #10, #16, #102, #104, #105, #106 | `機械証跡待ち` |
...
スクリプトは `commands.txt` と `summary.md` を出力し、由来ガード、イベント時刻への Date epoch 混入ガード、debug / release build、core tests、app bundle 作成と検証、bundle identity exact check、`LSUIElement=false` の通常 GUI app check、artifact 配下 config を使った `gui-smoke --json --assert` による AppKit 内 GUI smoke、未署名時の署名必須検証の期待失敗、ad-hoc 署名、署名済み bundle 検証、ライセンス原本一致、doctor、`doctor --json` の `runtimeReadiness` / `tccStatus` / `permissionTarget` / `grantRequired` / `targetDeviceDiagnostics` 存在確認、`doctor --probe-hid`、`doctor --assert-runtime-ready` の期待失敗と failure code、`benchmark --assert-baseline`、benchmark JSON の `schemaVersion: 3` / batch p95 / p99 field check、system-test dry-run と `analyze-log --assert-current-uptime --assert-system-scenario`、生成スクロール dry-run の current uptime assertion、派生値overflow・後続timestamp変換不能のexpected failureとstdout空判定、epoch fixture expected failure、fixture 解析、`derive-parameters --json --assert-complete`、`devices --all --json` を収集します。
...
`system-test run --scenario page-back/page-forward/zoom-in/zoom-out --dry-run --log-json` と `analyze-log --json --assert-current-uptime --assert-system-scenario <name>` は、Safari や対応アプリで実操作する前に、起動後単調時刻、`systemTestScenario`、`sequenceIndex`、離散割り当ての修飾キー down/up を含む完全な key event 列、keyCode、余計な modifier を含まない exact modifier flags を同じ JSON Lines 形式と終了コードで記録します。
`Fixtures/epoch-timestamp-generated-scroll-log.jsonl` の `analyze-log --json --assert-current-uptime` は expected failure とし、Unix epoch ナノ秒を起動後単調時刻として採用しないことを記録します。current uptime assertion はログ取得直後にだけ使います。
