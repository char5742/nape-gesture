...
`pr101-ax-delivery-final6` の top-level Safari 結果は nested scroll / wheel semantics の完成証跡にはしません。review 修正後は [Safari scroll 配送比較](safari-scroll-delivery-verification.md) を正とし、generic overflow の outer 誤配送防止と AX accessible frame の inner-only 更新までを実証済み、公開 AX tree が container を省略する場合の通常 wheel 同等挙動と Issue #102 取り込み後の最終再取得を未完了とします。
...
| ページ戻る / 進む / ズーム / 縦横スクロール | Safari または Reference Target App でのシナリオ別ログ、画面挙動メモ、割り当てとパラメータ | `system-test run --scenario page-back/page-forward/zoom-in/zoom-out/horizontal-scroll --dry-run --log-json`、`analyze-log --assert-system-scenario`、権限付与済み `.app` bundle の `generate-scroll --mode horizontal/free`、`AXScrollTargetSelector` / `AXScrollValuePlanner` core test、repo Safari fixture、Computer Use、`scripts/probe-cgevent-scroll-delivery.swift`、CLI expected failure | top-level 縦横と AX accessible nested frame は機械確認済み。AX が generic overflow 境界を省略する場合の通常 wheel 同等 target / `preventDefault()` semantics は未成立。Issue #102 取り込み後に最終 Safari / runtime 証跡を再取得する | `machine-evidence-pr59-main-dddb583/system-test-dry-run/system-page-*`, `machine-evidence-pr59-main-dddb583/system-test-dry-run/system-zoom-*`, `machine-evidence-pr59-main-dddb583/system-test-dry-run/system-horizontal-scroll*`, 旧 final6 top-level `scrollX 0 -> 1438 / scrollY 0 -> 1675`, 修正後 generic `outer=0 / inner=0 / wheel=0`, AX frame `outer=0 / frame=367 / frameWheel=0`, Computer Use generic `outer=0 / inner=682 / wheel=1`, top-level `scrollX=679 / scrollY=304`, [Safari scroll 配送比較](safari-scroll-delivery-verification.md), [ADR-0035](adr/0035-browser-discrete-shortcut-posting.md), [ADR-0036](adr/0036-scroll-event-target-process-routing.md) | #10, #16 | `一部完了` |
