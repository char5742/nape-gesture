...
`sample-cpu --expected-executable <path> --json --assert-baseline` は、指定 PID の実行ファイル、開始時刻トークン、audit token の `(pid, pidversion)` を各 sample の前後で固定し、CPU 使用率を `processCpuSampling` として保存します。`expectedExecutablePath`、`resolvedExecutablePath`、`executableIdentityMatched`、`processStartToken`、`processIDVersion`、`processIdentityStable` の不一致や取得不能は CPU 平均が基準内でも不合格です。開始トークン変化、pidversion 変化、path 変化はそれぞれ PID 再利用、`exec` / `posix_spawn`、実行ファイル変化として理由を分けます。completion evidence の短時間 smoke は、直接起動した `/bin/sleep` と `$!` の組み合わせによるコマンド形式の退行確認であり、日常利用主体の常駐 CPU 完了証跡ではありません。
