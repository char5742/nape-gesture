...
- runtime recovery readiness root: `artifacts/completion/2026-07-10/runtime-recovery-readiness`
- runtime recovery readiness JSON: `artifacts/completion/2026-07-10/runtime-recovery-readiness/recovery-readiness/recovery-readiness.json`
- runtime recovery readiness Markdown: `artifacts/completion/2026-07-10/runtime-recovery-readiness/recovery-readiness/recovery-readiness.md`
...
NAPE_COMPLETION_ARTIFACT_ROOT=artifacts/completion/2026-07-10/runtime-recovery-readiness sh scripts/collect-completion-evidence.sh
...
`runtime-recovery-readiness/summary.md` では、由来ガード、debug / release build、core tests、`.app` 作成と検証、doctor、`recovery-readiness --json --assert`、`recovery-readiness --markdown --assert`、system-test dry-run、fixtures 解析、`devices --all --json` まで成功している。
...
| スリープ復帰 / 抜き差し / 権限変更後復旧 | スリープ復帰、Nape Pro 抜き差し、TCC 権限変更後の停止、再試行、復旧ログ | `recovery-readiness --json --assert`、`RuntimeRecoveryState` の回帰テスト、`RuntimeStatusPresenter` の UI 表示回帰テスト、`PermissionRecoveryPresenter` の権限別復旧導線テスト、`doctor --probe-hid --json`、設定検証、権限未許可時のエラー出力 | Mac のスリープ復帰、実デバイス抜き差し、システム設定での TCC 権限変更 | `machine-evidence-pr59-main-dddb583/build-and-tests/core-tests.log`, `runtime-event-open-absolute-paths-final/doctor/doctor-debug.json`, `recovery-readiness/recovery-readiness.json`, [runtime event 最新コメント](https://github.com/char5742/nape-gesture/issues/16#issuecomment-4926725190), [runtime recovery readiness](runtime-recovery-readiness.md) | #13, #16, #74 | `人間作業待ち` |
...
`recovery-readiness --json --assert` は、Issue #13 のスリープ復帰、対象デバイス抜去、対象デバイス再接続、アクセシビリティ変更、入力監視変更、回復可能 runtime 失敗、人間修正が必要な失敗をシナリオ化し、機械で固定済みの契約と残る外部証跡を同じ JSON に出します。
このコマンドは `schemaVersion: 1`、`reportKind: "runtimeRecoveryReadiness"`、`needHumanLabelCandidateScenarioCount`、`externalEvidenceRequired` を持ち、外部証跡が必要なシナリオを `completed` にしないことも `--assert` で確認します。
これはスリープ、抜き差し、TCC 変更の実ログを代替しません。実機確認へ進むときは [Runtime recovery readiness](runtime-recovery-readiness.md) のシナリオ名に合わせて runtime ログ、常駐 UI 観測、復旧後 `doctor`、target log を保存します。
