...
| Trackpad output event logger | 純正trackpadのevent type / subtype / raw field / serialized dataをJSON Linesへ保存し、確定captureをmanifestへ固定できる | `trackpad-event-log --duration --out --evidence-kind`、capture順、ordered raw field、非有限値bit pattern、bounded queue、SIGINT drain、0件期待失敗、最終log / executable SHA、atomic manifest | なし | [2026-07-11 local smoke](evidence/2026-07-11-trackpad-event-logger-local-smoke.md)。manifest追加後の統合証跡はPRで更新する | #117, #118, #129, #132 | `一部完了` |
| Trackpad raw analyzer Phase 1 | 現行JSON Lines、manifest、serialized event、generated配送を機械判定できる | 厳格parser negative tests、nesting上限、nullable subtype、unknown field保持、manifest原子書き込みと改ざん検出、CGEvent再構築、生成marker / actual type / raw target process field、provenanceのSHA / 件数 / 順序 / timestamp / type一致、製品source境界guard、CLI正常系と非ゼロ終了 | なし | core testsとsynthetic serialized eventのlocal CLI smoke。純正fixture値のcontract比較は#125後 | #117, #125, #129 | `一部完了` |
...
| 純正トラックパッド比較 | 純正trackpadと生成eventを同一schemaで保存したtype、subtype、field、phase、momentum、timestamp、順序の比較結果 | logger / analyzer、fixture contract test、差分report | 純正trackpadでscroll、横scroll、momentum停止、Spaces、Mission Control、page swipe、pinchを操作する | 既存`sample-trackpad-scroll-log`は公開scroll fieldの参考専用。raw上位output contractは未設定 | #7, #8, #117, #118, #125, #129 | `機械証跡待ち` |
| Trackpad scroll / momentum | continuous scroll eventとcompanion gesture eventの同一timestamp系列、phase完結、momentum分離 | output model test、CGEvent contract test、Safari / Reference Targetの自動受信log | Nape Proと純正trackpadで最終体感比較する | 既存単純scroll証跡は参考専用。新contract証跡は未設定 | #7, #117, #119, #125 | `機械証跡待ち` |
| Spaces / Mission Control | DockSwipeのmotion、progress、phase、終了速度、画面連続追従、stuckなし | DockSwipe model / adapter test、version fixture、system-wide output log | 純正trackpadとNape ProでSpaces / Mission Control / App Exposéを操作する | 既存forced scroll / shortcut証跡は参考専用。DockSwipe証跡は未設定 | #9, #117, #122, #125, #126, #128, #130 | `機械証跡待ち` |
| ページ戻る / 進む / ズーム / 横スクロール | NavigationSwipe、magnification / zoom、trackpad horizontal scrollのevent contractと通常OS処理結果 | output model / adapter test、Reference Target App、Safari runtime log | 純正trackpadとNape Proでpage swipe、pinch、横scrollを操作する | 既存生成key /単純scroll証跡は参考専用。新contract証跡は未設定 | #10, #117, #119, #122, #125, #127, #128, #130 | `機械証跡待ち` |
