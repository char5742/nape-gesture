...
`sh scripts/check-provenance.sh` は、外部ソースを読まずに tracked files だけを対象とし、既知の第三者プロジェクト固有名の表記揺れを allowlist なしで検出し、README / PR template / PR review checklist の一般化した由来方針維持を確認します。`THIRD_PARTY_NOTICES.md`は実際に同梱する依存通知に限定します。このチェックは法的な完全証明ではなく、由来混入の早期検出ガードとして扱います。
