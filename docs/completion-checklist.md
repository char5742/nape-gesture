...
`system-test-pid-target-main-746df2c/summary.md` では、`space-left`、`space-right`、`horizontal-scroll` の dry-run、`analyze-log --assert-system-scenario`、Reference Target App PID への直接投稿、`analyze-target-log --assert-has-generated-event --assert-has-foreground-capture` が成功しています。
