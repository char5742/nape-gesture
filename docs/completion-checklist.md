...
- `.app` を通常起動し、Dock 表示とステータスアイテム `NG` メニューを確認する。設定ウィンドウ表示、保存操作、通常アプリメニューの権限導線は computer-use 証跡で代替済み。
