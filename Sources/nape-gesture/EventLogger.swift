...
final class EventLogger {
    ...
    private func shouldEmit(_ record: InputLogRecord) -> Bool {
        if configuration.excludesGeneratedEvents && record.generatedByNapeGesture {
            return false
        }
        if configuration.onlyGeneratedEvents && !record.generatedByNapeGesture {
            return false
        }
        return true
    }
...
extension InputLogRecord {
    init(type: CGEventType, event: CGEvent) {
        self.init(
            ...
            generatedByNapeGesture: CGEventUtilities.isGeneratedByThisTool(event),
