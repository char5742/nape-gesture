        ...
        print(
            ...
            使い方:
              ...
              nape-gesture run [--performance-log <path>]
                  button 3 / 4 / 5の押下中入力を、設定された2本指スクロール / スワイプ、3本指システムスワイプ、4本指システムピンチへ変換します。対象button未押下時は通常mouse入力をそのまま通します。
                  --config <path> ではbutton 3 / 4 / 5のGestureClass割り当て、3本指 / 4本指の共有感度、対象デバイス、安全用キャンセル条件を読み込みます。--performance-log で runtime 性能 JSON Lines を保存します。
              ...
              nape-gesture analyze-trackpad-event-log <log.jsonl> --manifest <manifest.json> [--provenance <trace.jsonl>] [--contract <scroll-momentum-contract.json>] [--json]
                  現行 raw schema、capture manifest、serialized CGEvent 再構築、generated product の system-wide 配送 provenance、登録済み scroll / momentum contract を検証します。不正時も report を出して非ゼロ終了します。
