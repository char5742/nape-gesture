    ...
    private func printHelp() {
        print(
            ...
            使い方:
              ...
              nape-gesture analyze-log <path> [--json] [--assert-has-unmarked-passthrough-input] [--assert-has-unmarked-click] [--assert-has-unmarked-drag] [--assert-has-unmarked-wheel] [--assert-has-unmarked-click-drag-wheel] [--assert-kill-switch-shortcut] [--assert-gesture-before-kill-switch] [--assert-generated-scroll-log --expected-direction positive-x|negative-x --expected-normal-events <数> --expected-momentum-events <数> --expected-normal-x-total <量> --expected-phase-mode auto] [--assert-system-scenario <name>]
                  JSON Lines ログを解析し、しきい値候補を出します。--assert-has-unmarked-passthrough-input で未生成の移動またはスクロールがない場合、--assert-has-unmarked-click / --assert-has-unmarked-drag / --assert-has-unmarked-wheel で未生成の通常クリック / 通常ドラッグ / 通常ホイールがない場合、--assert-kill-switch-shortcut で未生成の Control + Option + Command + G keyDown / keyUp がない場合、--assert-gesture-before-kill-switch でキルスイッチ前の未生成ジェスチャー入力がない場合、--assert-generated-scroll-log と全 expected オプションで generate-scroll auto dry-run の方向、通常件数、momentum changed 件数、通常 X 合計量、per-step 量子化、exact phase 列を満たさない場合、--assert-system-scenario で system-test dry-run の期待イベント列を満たさない場合に失敗します。通常件数は1以上、momentum件数は0以上を受理し、生成スクロール assertion の明示 phase override は未対応です。
