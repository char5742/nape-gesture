...
final class MacGestureDaemon {
    ...
    func run() throws {
        try start()
        print("mac-gesture を開始しました。停止するには Ctrl-C を押してください。")
...
private let eventTapCallback: CGEventTapCallBack = { proxy, type, event, userInfo in
    ...
    let daemon = Unmanaged<MacGestureDaemon>.fromOpaque(userInfo).takeUnretainedValue()
