...
enum SettingsStore {
    ...
    static func load(from options: [String]) throws -> MacGestureSettings {
    ...
    static func loadRuntimeSettings(from options: [String], validate: Bool = true) throws -> (settings: MacGestureSettings, path: String) {
    ...
    static func load(fromPath path: String) throws -> MacGestureSettings {
        let data = try Data(contentsOf: URL(fileURLWithPath: path))
        return try decoder.decode(MacGestureSettings.self, from: data)
    }

    static func loadOrCreateDefault(at path: String) throws -> MacGestureSettings {
    ...
    static func writeTemplate(to path: String) throws {
        try write(MacGestureSettings.template, to: path)
    }

    static func write(_ settings: MacGestureSettings, to path: String) throws {
        let url = URL(fileURLWithPath: path)
        try FileManager.default.createDirectory(
            at: url.deletingLastPathComponent(),
            withIntermediateDirectories: true
    ...
    static func defaultConfigPath() -> String {
        ...
        return base
            .appendingPathComponent("MacGesture", isDirectory: true)
            .appendingPathComponent("config.json")
            .path
    }

    static func templateString() throws -> String {
        try string(for: MacGestureSettings.template)
    }

    static func string(for settings: MacGestureSettings) throws -> String {
        let data = try encoder.encode(settings)
    ...
    static func validateSettings(_ settings: MacGestureSettings) throws {
