...
final class CommandLineTool {
    ...
    private func printHelp() {
        print(
            ...
            使い方:
              ...
              mac-gesture log [--duration <秒>] [--out <path>] [--exclude-generated|--only-generated]
                  グローバル入力イベントを JSON Lines で記録します。メタ情報は標準エラー、イベント本体は標準出力または --out に出します。

              mac-gesture analyze-log <path> [--json]
                  JSON Lines ログを解析し、しきい値候補を出します。

              mac-gesture compare-log <baseline> <candidate> [--json]
                  純正入力ログと生成イベントログなど、2つの JSON Lines ログ差分を比較します。

              mac-gesture analyze-hid-log <path> [--json]
                  IOHID 生入力ログを device / usage ごとに集計します。

              mac-gesture analyze-target-log <path> [--json]
                  Reference Target App が保存した AppKit 受信イベントを集計します。

              mac-gesture check-config [--config <path>] [--probe-hid]
                  対象デバイス設定と HID 入力監視の開始可否を確認します。

              mac-gesture config-path [--config <path>]
                  使用する設定ファイルのパスを表示します。

              mac-gesture bundle-app [--out <path>] [--replace]
                  現在の実行ファイルから .app バンドルを作成します。

              mac-gesture verify-bundle <path>
                  .app バンドルの Info.plist、実行ファイル、配布文書を検証します。

              mac-gesture generate-scroll --x <値> --y <値> [--steps <数>] [--phase auto|began|changed|ended|cancelled|momentum] [--momentum-steps <数>] [--dry-run] [--json|--log-json]
                  ピクセル単位のスクロールイベントを任意フェーズや慣性つきで生成します。--dry-run --log-json で compare-log 用 JSON Lines を出力します。

              mac-gesture init-config [--out <path>] [--vendor-id <ID>] [--product-id <ID>] [--manufacturer-contains <文字>] [--product-contains <文字>] [--transport-contains <文字>] [--usage-page <ID>] [--usage <ID>] [--allow-unmatched]
                  Nape Pro 向け、または指定した HID 条件向けの設定テンプレートを出力します。

              mac-gesture target [--out <path>]
                  AppKit が受け取るイベントを表示する基準ウィンドウを開きます。--out で受信イベントを JSON Lines に保存します。

              mac-gesture devices [--json] [--all]
                  IOHID で認識できるデバイスを表示します。通常はマウス系だけ、--all で全HIDを表示します。

              mac-gesture hid-log [--duration <秒>] [--vendor-id <ID>] [--product-id <ID>] [--usage-page <ID>] [--usage <ID>] [--all]
                  IOHID の生入力を JSON Lines で記録します。vendor/product/usage 指定時はその条件だけを開きます。

              mac-gesture system-test list
                  System Behavior Test のシナリオ一覧を表示します。

              mac-gesture system-test run --scenario <name> [--target finder|safari] [--dry-run] [--log-json] [--out <path>]
                  Spaces / Mission Control / Safari / Finder 向けの実挙動検証イベントを生成します。--dry-run --log-json で計画した生成イベントを JSON Lines で出力します。

              mac-gesture benchmark [--events <数>] [--json]
                  認識器とスクロール計画の純粋ロジック処理時間を測定します。

              mac-gesture doctor [--config <path>] [--probe-hid] [--benchmark-events <数>] [--json]
    ...
    private func makeInitialSettings(options: [String]) throws -> MacGestureSettings {
    ...
    private func probeHIDInputMonitor(settings: MacGestureSettings, matchedDevices: [DeviceIdentity]) throws {
...
enum ToolError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .targetDeviceNotFound:
            return "設定に一致する対象デバイスが見つかりませんでした。`mac-gesture devices` で識別情報を確認してください。"
...
enum IOReturnDiagnostic {
    static func describe(_ code: IOReturn) -> String {
        ...
        case kIOReturnNotPermitted:
            return "入力監視が許可されていません。システム設定 > プライバシーとセキュリティ > 入力監視で、Codex、実行元ターミナル、または MacGesture.app を許可してください。"
